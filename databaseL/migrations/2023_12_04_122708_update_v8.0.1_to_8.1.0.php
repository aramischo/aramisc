...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('aramiscedu__pages', function (Blueprint $table) {
            if (!Schema::hasColumn('aramiscedu__pages', 'is_default')) {
       ...
       // $in_app_live = Permission::where('module','InAppLiveClass')->delete();
        ...
        foreach ($filesInFolder as $path) {
            ...
            if (file_exists($file['dirname'] . '/' . $file['basename'])) {
                ...
                if ($file_data) {
                    $check_exist  = DB::table(config('pagebuilder.db_prefix', 'aramiscedu__') . 'pages')->where('school_id', 1)->where('slug', $file_data['slug'])->first();
                    if (!$check_exist) {
                        DB::table(config('pagebuilder.db_prefix', 'aramiscedu__') . 'pages')->insert(
                            [
                                'name' => $file_data['name'],
