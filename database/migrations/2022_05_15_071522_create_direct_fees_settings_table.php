...
class CreateDirectFeesSettingsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('direct_fees_settings', function (Blueprint $table) {
            ...
            $table->foreign('school_id')->on('aramisc_schools')->references('id')->cascadeOnDelete();
            $table->timestamps();
        });
        try {
            ...
            foreach ($schools as $school) {
                ...
                $new->academic_id = 1;
                $new->save();
                Schema::table('aramisc_general_settings', function (Blueprint $table) use ($module_name) {
                    if (!Schema::hasColumn('aramisc_general_settings', $module_name)) {
                        $table->unsignedBigInteger($module_name)->nullable();
                    }

                    if (!Schema::hasColumn('aramisc_general_settings', 'direct_fees_assign')) {
            ...
            Schema::table('aramisc_fees_masters', function (Blueprint $table) use ($class_id) {
                if (!Schema::hasColumn('aramisc_fees_masters', $class_id)) {
                    $table->unsignedBigInteger($class_id)->nullable();
                }
            });

            Schema::table('aramisc_fees_masters', function (Blueprint $table) use ($section_id) {
                if (!Schema::hasColumn('aramisc_fees_masters', $section_id)) {
                    $table->unsignedBigInteger($section_id)->nullable();
                }
            });


            Schema::table('aramisc_fees_assigns', function (Blueprint $table) use ($class_id) {
                if (!Schema::hasColumn('aramisc_fees_assigns', $class_id)) {
                    $table->unsignedBigInteger($class_id)->nullable();
                }
            });

            Schema::table('aramisc_fees_assigns', function (Blueprint $table) use ($section_id) {
                if (!Schema::hasColumn('aramisc_fees_assigns', $section_id)) {
                    $table->unsignedBigInteger($section_id)->nullable();
                }
            });

            Schema::table('aramisc_fees_payments', function (Blueprint $table) use ($direct_fees_installment_assign_id) {
                if (!Schema::hasColumn('aramisc_fees_payments', $direct_fees_installment_assign_id)) {
            ...
            if (moduleStatusCheck('ParentRegistration')) {
                ...
                foreach ($columns as $column) {
                    Schema::table('aramisc_student_registrations', function (Blueprint $table) use ($column) {
                        if (!Schema::hasColumn('aramisc_student_registrations', $column)) {
