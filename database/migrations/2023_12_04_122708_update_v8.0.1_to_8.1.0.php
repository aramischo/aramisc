...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('aramisc_video_galleries', function (Blueprint $table) {
            if (!Schema::hasColumn('aramisc_video_galleries', 'position')) {
                $table->integer('position')->default(0);
            }
        });

        Schema::table('aramisc_photo_galleries', function (Blueprint $table) {
            if (!Schema::hasColumn('aramisc_photo_galleries', 'position')) {
                $table->integer('position')->default(0);
            }
        });

        Schema::table('aramiscdu__pages', function (Blueprint $table) {
            if (!Schema::hasColumn('aramiscdu__pages', 'is_default')) {
                $table->boolean('is_default')->default(0);
            }
        });

        Schema::table('aramisc_expert_teachers', function (Blueprint $table) {
            if (!Schema::hasColumn('aramisc_expert_teachers', 'position')) {
                $table->integer('position')->default(0);
            }
            if (!Schema::hasColumn('aramisc_expert_teachers', 'staff_id')) {
                $table->tinyInteger('staff_id');
            }
            if (!Schema::hasColumn('aramisc_expert_teachers', 'created_by')) {
                $table->tinyInteger('created_by')->nullable();
            }
            if (!Schema::hasColumn('aramisc_expert_teachers', 'updated_by')) {
                $table->tinyInteger('updated_by')->nullable();
            }
            if (Schema::hasColumn('aramisc_expert_teachers', 'name')) {
                $table->dropColumn('name');
            }
            if (Schema::hasColumn('aramisc_expert_teachers', 'designation')) {
                $table->dropColumn('designation');
            }
            if (Schema::hasColumn('aramisc_expert_teachers', 'image')) {
                $table->dropColumn('image');
            }
        });

        DB::table('aramisc_expert_teachers')->truncate();

        Schema::table('aramisc_contact_messages', function (Blueprint $table) {
            if (!Schema::hasColumn('aramisc_contact_messages', 'phone')) {
                $table->string('phone')->nullable();
            }
        });
        
        Schema::table('aramisc_staffs', function (Blueprint $table) {
            if (!Schema::hasColumn('aramisc_staffs', 'show_public')) {
       ...
       // $in_app_live = Permission::where('module','InAppLiveClass')->delete();
        ...
        foreach ($filesInFolder as $path) {
            ...
            if (file_exists($file['dirname'] . '/' . $file['basename'])) {
                ...
                if ($file_data) {
                    $check_exist  = DB::table(config('pagebuilder.db_prefix', 'aramiscdu__') . 'pages')->where('school_id', 1)->where('slug', $file_data['slug'])->first();
                    if (!$check_exist) {
