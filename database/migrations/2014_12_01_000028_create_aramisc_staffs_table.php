...
class CreateAramiscStaffsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('aramisc_staffs', function (Blueprint $table) {
            $table->increments('id');
            ...
            $table->foreign('designation_id')->references('id')->on('aramisc_designations')->onDelete('set null');

            $table->integer('department_id')->nullable()->unsigned()->default(1);
            $table->foreign('department_id')->references('id')->on('aramisc_human_departments')->onDelete('set null');
    ...
    {
        Schema::dropIfExists('aramisc_staffs');
    }
}
