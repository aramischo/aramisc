...
class AramiscClassesTableSeeder extends Seeder
{
    public $sections;
    public $subjects;
    ...
    {
        $this->sections = AramiscSection::all();
        $this->subjects = AramiscSubject::all();
    }
    ...
    {
        $sections = AramiscSection::where('school_id', $school_id)->where('academic_id', $academic_id)->get();
        $subjects = AramiscSubject::where('school_id', $school_id)->where('academic_id', $academic_id)->get();

        AramiscClass::factory()->times($count)->create([
            'school_id' => $school_id,
            ...
            foreach ($sections as $section) {
                ...
                $i = 0;
                AramiscStudent::factory()->times(50)->create()->each(function ($student) use ($class, $section, $school_id) {
                    ...
                    AramiscParent::factory()->times(1)->create([
                        'school_id' => $school_id,
                        'guardians_email' => 'guardian_' . $student->id . '@aramiscedu.com',
                    ...
                    foreach(lastOneMonthDates() as $date){
                        ...
                        $studentAttendance = new AramiscStudentAttendance();
            ...
            AramiscAdmissionQuery::factory()->times(10)->create([
                'class' => $class->id,
                'school_id' => $school_id,
                'academic_id' => $class->academic_id,
            ])->each(function ($admission_query) use($school_id) {
                AramiscAdmissionQueryFollowup::factory()->times(random_int(5, 10))->create([
