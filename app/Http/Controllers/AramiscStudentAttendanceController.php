...
class AramiscStudentAttendanceController extends Controller
{
    ...
    public function studentSearch(Request $request)
    {
        ...
        try {
            ...
            $students = AramiscStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('active_status', 1)->where('academic_id', getAcademicId())
        ...
        try {
            foreach ($request->id as $student) {
                ...
                if ($request->attendance[$student] == 'P') {
                    $student_info = AramiscStudent::find($student);
    ...
    {
        $students = AramiscStudent::where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('active_status', 1)->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)->get();
        if ($students->isEmpty()) {
            Toastr::error('No Result Found', 'Failed');
            return redirect('student-attendance');
                    ...
                    try {
                        foreach ($data as $key => $value) {
                            if ($value != "") {
                                ...
                                if (date('d/m/Y', strtotime($request->attendance_date)) == date('d/m/Y', strtotime($value->attendance_date))) {
                                    $student = AramiscStudent::select('id')->where('id', $value->student_id)->where('school_id', Auth::user()->school_id)->first();

                                    // return $student;

                                    if ($student != "") {
                                        // AramiscStudentAttendance
                                        $attendance_check = AramiscStudentAttendance::where('student_id', $student->id)
                        ...
                        //         $attendance_check=AramiscStudentAttendance::where('student_id',$all_student_id)->where('attendance_date',date('Y-m-d', strtotime($value->attendance_date)))->first();
                        //         if ($attendance_check) {
                        //            $attendance_check->delete();
                        //         }
                        //         $import = new AramiscStudentAttendance();
