...
class AramiscStudentIdCardController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $id_card = new AramiscStudentIdCard();
        ...
        try {
            ...
            $roles = AramiscRole::select('*')->where('id', '!=', 1)->where(function ($q) {
             ...
             if (checkAdmin()) {
                ...
                $id_card = AramiscStudentIdCard::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->file('logo') != "") {
                ...
                // $id_card = AramiscStudentIdCard::find($request->id);
                // if ($id_card->logo != "") {
            ...
            if ($request->file('profile_image') != "") {
             ...
             if (checkAdmin()) {
                ...
                $id_card = AramiscStudentIdCard::where('id',$request->id)->where('school_id',Auth::user()->school_id)->first();
            }
            $id_card->title = $request->title;
            if ($fileNamelogo != "") {
                $id_card->logo = $fileNamelogo;
    ...
    public function ajaxIdCard(Request $request){
            ...
            $id_cards = AramiscStudentIdCard::status()->get();
            $idCards=[];
    ...
    public function generateIdCardSearch(Request $request)
    {
        ...
        try {
            ...
            $students = AramiscStudent::with('class','parents','section','gender')->where('active_status', 1)
    ...
    public function generateIdCardPrint($s_id, $c_id)
    {
        ...
        try {
            ...
            foreach ($s_ids as $sId) {
                $students[] = AramiscStudent::find($sId);
            }

           

            $id_card = AramiscStudentIdCard::find($c_id);
