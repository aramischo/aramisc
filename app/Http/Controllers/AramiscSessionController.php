...
class AramiscSessionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try{
            $sessions = AramiscSession::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
    ...
    {
        try{
            $session = AramiscSession::find($id);
            $sessions = AramiscSession::where('school_id',Auth::user()->school_id)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        try{
            $session = AramiscSession::find($request->id);
            $session->session = $request->session;
            $result = $session->save();
    ...
    public function destroy(Request $request, $id)
    {
        ...
        try {
            $delete_query = AramiscSession::destroy($id);
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                if ($delete_query) {
        ...
        // $session = AramiscSession::destroy($id);
        // if (ApiBaseMethod::checkUrl($request->fullUrl())) {
        //     if ($session) {
        //         return ApiBaseMethod::sendResponse(null, 'Session has been deleted successfully');
        //     } else {
