...
class AramiscFeesController extends Controller
{
    ...
    public function studentFees()
    {
        try {
            ...
            $paystack_info = DB::table('aramisc_payment_gateway_settings')->where('gateway_name', 'Paystack')
                                        ->where('school_id', Auth::user()->school_id)->first();

            $account_info = AramiscPaymentMethhod::whereIn('method', ['Bank','Cheque'])->where('school_id',auth()->user()->school_id)->get();
            $data['bank_info'] = $account_info->where('method', 'Bank')->first();
            $data['cheque_info'] = $account_info->where('method', 'Cheque')->first();
            $gateway = AramiscPaymentGatewaySetting::where('service_charge', 1)->get(['gateway_name', 'charge_type', 'charge']);
            $method['Stripe'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Stripe')->where('school_id', Auth::user()->school_id)->first();
            $invoice_settings = FeesInvoice::where('school_id',auth()->user()->school_id)->first();
            $razorpay_info = AramiscPaymentGatewaySetting::where('gateway_name', 'RazorPay')->where('school_id', Auth::user()->school_id)->first();
    ...
    {
        try {
            $paystack_info = DB::table('aramisc_payment_gateway_settings')->where('gateway_name', 'Paystack')->where('school_id', Auth::user()->school_id)->first();
    ...
    public function handleGatewayCallback()
    {
        try {
            ...
            if(! moduleStatusCheck('University')){
                ...
                ->where('aramisc_fees_masters.fees_type_id',$fees_payment->fees_type_id)
                ->where('aramisc_fees_assigns.student_id',$fees_payment->student_id)->first();

                $fees_assign=AramiscFeesAssign::where('fees_master_id',$get_master_id->fees_master_id)->where('student_id',$fees_payment->student_id)->where('school_id',Auth::user()->school_id)->first();
    ...
    public function feesPaymentStripeStore(Request $request)
    {
        ...
        if(moduleStatusCheck('University')){
            ...
            $get_master_id=AramiscFeesMaster::join('aramisc_fees_assigns','aramisc_fees_assigns.fees_master_id','=','aramisc_fees_masters.id')
                ->where('aramisc_fees_masters.fees_type_id',$request->fees_type)
                ->where('aramisc_fees_assigns.student_id',$request->student_id)->first();
    ...
    public function directFeesGenerateModalChild(Request $request, $amount , $installment_id, $record_id)
    {
        try {
            ...
            $data['cheque_info'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Cheque')->where('school_id', Auth::user()->school_id)->first();
            $method['cheque_info'] = AramiscPaymentMethhod::where('method', 'Cheque')->where('school_id', Auth::user()->school_id)->first();
            $data['PayPal'] = AramiscPaymentMethhod::where('method', 'PayPal')
                            ->where('school_id', Auth::user()->school_id)
                            ->first('active_status');
            $data['Stripe'] = AramiscPaymentMethhod::where('method', 'Stripe')
                                ->where('school_id', Auth::user()->school_id)
                                ->first('active_status');
            $data['Paystack'] = AramiscPaymentMethhod::where('method', 'Paystack')
                                ->where('school_id', Auth::user()->school_id)
                                ->first('active_status');
            if(moduleStatusCheck('CcAveune')) {
                $data['CcAveune'] = AramiscPaymentMethhod::where('method', 'CcAveune')
                                    ->where('school_id', Auth::user()->school_id)
                                    ->first('active_status');
            }   
            
            if(moduleStatusCheck('PhonePay')) {
                $data['PhonePe'] = AramiscPaymentMethhod::where('method', 'PhonePay')
                                    ->where('school_id', Auth::user()->school_id)
                                    ->first('active_status');
            } 

            $method['PayPal'] = AramiscPaymentGatewaySetting::where('gateway_name', 'PayPal')->where('school_id', Auth::user()->school_id)->first();
            $method['Stripe'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Stripe')->where('school_id', Auth::user()->school_id)->first();
            $method['Paystack'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Paystack')->where('school_id', Auth::user()->school_id)->first();
            $method['PhonePe'] = AramiscPaymentGatewaySetting::where('gateway_name', 'PhonePay')->where('school_id', Auth::user()->school_id)->first();
    ...
    public function directFeesTotalPayment($record_id){
        ...
        try{
            ...
            if(moduleStatusCheck('PhonePay')) {
                $data['PhonePay'] = AramiscPaymentMethhod::where('method', 'PhonePay')
                ->where('school_id', Auth::user()->school_id)
                ->first('active_status');
            }   
