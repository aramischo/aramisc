...
class AramiscOnlineExamController extends Controller
{
    ...
    {
        try {
            $online_exam_info=AramiscOnlineExam::find($request->online_exam_id);
            // dd($online_exam_info);
            if (moduleStatusCheck('University')) {
                ...
                $teacher_info=AramiscAssignSubject::where('class_id',$online_exam_info->class_id)
            ...
            if ($online_exam_info->auto_mark==1) {
                $question_types=DB::table('aramisc_online_exam_question_assigns')->where('online_exam_id',$request->online_exam_id)
                ->leftjoin('aramisc_question_banks','aramisc_question_banks.id','=','aramisc_online_exam_question_assigns.question_bank_id')
                ->get();
    ...
    public function takeOnlineExam($id)
    {
        try {
            ...
            // } else {
            //     $assigned_questions = AramiscOnlineExamQuestionAssign::where('online_exam_id', $online_exam->id)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            //     return view('backEnd.studentPanel.take_online_exam', compact('online_exam', 'assigned_questions'));
            // }
    ...
    public function autoMarking($online_exam_id, $student_id, $question_id, $user_choose,$ajax_post)
    {
        ...
        $exam_info = AramiscOnlineExam::find($online_exam_id);
        ...
        if ($question_info->type == 'MI') {
            if ($question_info->answer_type=='radio') {
                ...
                $currect_answers =AramiscQuestionBankMuOption::where('question_bank_id',$question_info->id)->where('status', 1)
                ->get();
    ...
    function universityStudentOnlineExamSubmit($request){
        ...
        try {
            $online_exam = AramiscOnlineExam::findOrFail($request->online_exam_id);
            ...
            foreach ($request->question_ids as $key => $question_id) {
                $question_bank = AramiscQuestionBank::find($question_id);
                ...
                if ($question_bank->type == "T") {
                    ...
                    $question_options = AramiscQuestionBankMuOption::where('question_bank_id', $question_bank->id)
                ...
                if ($online_take_exam_mark) {
                    ...
                    if (isset($request->marks)) {
                        foreach ($request->marks as $mark) {
                            $question_marks = AramiscQuestionBank::select('marks')->where('id', $mark)->where('un_academic_id', getAcademicId())->first();
            ...
            try {
                ...
                if (!in_array("F", $question_types) && $online_exam_info->auto_mark == 1) {
                    ...
                    if ($online_take_exam_mark) {
                        ...
                        if (isset($request->marks)) {
                            foreach ($request->marks as $mark) {
                                $question_marks = AramiscQuestionBank::select('marks')->where('id', $mark)->where('academic_id', getAcademicId())->first();
                                $total_marks = $total_marks + $question_marks->marks;
                            }
                        }
                        $online_take_exam_mark->total_marks = $total_marks;
    ...
    public function AjaxStudentOnlineExamSubmit(Request $request)
    {
        ...
        $online_exam = AramiscOnlineExam::find($request->online_exam_id);
        $question_info = AramiscQuestionBank::find($request->question_id);
        $student = Auth::user()->student;
        ...
        if ($now >= $endTime) {
            ...
            $question_bank = AramiscQuestionBank::find($request->question_id);
    ...
    { 
        try {
            ...
            $assigned_question_list=[];
               $assigneds= AramiscOnlineExamQuestionAssign::where('online_exam_id', $exam_id)->select('question_bank_id')->get();
           ...
           $skipped_questions=[];
           if ($array_diff!=null) {
                $skipped_questions=AramiscOnlineExamQuestionAssign::whereIn('question_bank_id', array($array_diff))->get();
           }
        //    $skipped_questions=AramiscQuestionBank::whereIn('id', array($array_diff))->get();
        ...
        try {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                $online_exams = AramiscOnlineExam::where('aramisc_online_exams.status', '=', 1)
                    ...
                    ->where('aramisc_online_exams.school_id', Auth::user()->school_id)->get();
