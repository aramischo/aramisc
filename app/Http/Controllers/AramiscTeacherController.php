...
class AramiscTeacherController extends Controller
{
    ...
    {
        $editData = AramiscTeacherUploadContent::where('school_id', Auth::user()->school_id)
        ->where('academic_id',getAcademicId())
        ->where('id',$id)
        ->first();
       ...
       // DB::statement('SET FOREIGN_KEY_CHECKS=0;');
        ...
        try {
            ...
            if($request->section == "all"){
                $sections = AramiscClassSection::where('class_id', $request->class)               
            ...
            foreach ($request->available_for as $value) {
                ...
                if ($value == 'student') {
                    ...
                    elseif ( (!is_null($request->class)) &&   ($request->section == '')) {
                        ...
                        $students = AramiscStudent::select('id', 'user_id')->where('class_id', $request->class)->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
                        foreach ($students as $student) {
                            $notification = new AramiscNotification;
                            $notification->user_id = $student->user_id;
                            $notification->role_id = 2;
                        ...
                        $students = AramiscStudent::select('id','user_id')->where('class_id', $request->class)->where('section_id', $request->section)->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
                        foreach ($students as $student) {
                            $notification = new AramiscNotification;
                            $notification->user_id = $student->user_id;
                            $notification->role_id = 2;
            ...
            foreach ($request->available_for as $value) {
                ...
                if ($value == 'student') {
                    if (isset($request->all_classes)) {
                        ...
                        $students = AramiscStudent::select('id')->where('class_id', $request->class)->where('section_id', $request->section)->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
                        foreach ($students as $student) {
                            $notification = new AramiscNotification;
                            $notification->user_id = $student->id;
                            $notification->role_id = 2;
    ...
    public function assignmentList(Request $request)
    {
        ...
        try{
            ...
            if (!teacherAccess()) {
                    ...
                    $uploadContents = AramiscTeacherUploadContent::where('content_type', 'as')->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
