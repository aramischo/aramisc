...
use App\Http\Controllers\Admin\StudentInfo\AramiscStudentAdmissionController;
use App\Models\Graduate;
use Illuminate\Support\Facades\Validator;

class AramiscStudentPromoteController extends Controller
{
    //
    ...
    public function studentCurrentSearch(Request $request)
    {
        ...
        try {
            ...
            $students = AramiscStudent::query()->with('class', 'section');
    ...
    public function rollCheck(Request $request)
    {
        ...
        $exist_roll_number = AramiscStudent::where('class_id', $request->class_id)
    ...
    // public function old_promote(Request $request)
        ...
        //                 $current_student = AramiscStudent::where('id', $student_id)->first();
        ...
        //                         $insertStudentRecord = new AramiscStudentAdmissionController;
    ...
    public function studentSearchWithExam(Request $request)
    {
        ...
        try {
            ...
            $search_current_academic_year = AramiscAcademicYear::find($request->current_session);
            $search_promote_academic_year = AramiscAcademicYear::find($request->promote_session);
            $search_exams = AramiscExamType::find($request->exam)->title;
            $sections = $search_current_class ? $search_current_class->classSection : [];
            $exams = AramiscExamType::where('active_status', 1)->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)->get();
