...
class DatatableQueryController extends Controller
{
    public function studentDetailsDatatable(Request $request)
    {
        ...
        if ($request->ajax()) {
           ...
           $all_students =  AramiscStudent::with('studentRecords','studentRecords.class','studentRecords.section')->whereIn('id',$student_records)
            ...
            }])->select('aramisc_students.*');
            $students->where('aramisc_students.active_status', 1);

            // if ($request->name != "") {
            //     $students->where('aramisc_students.full_name', 'like', '%' . $request->name . '%');
            // }
            // if ($request->roll_no != "") {
            //     $students->where('aramisc_students.roll_no', 'like', '%' . $request->roll_no . '%');
            // }

            // return $request;
            $students = $students->with('studentRecords','studentRecords.class','studentRecords.section')->where('aramisc_students.school_id', Auth::user()->school_id)
                ->with(array('parents' => function ($query) {
                    $query->select('id', 'fathers_name');
                }))
                ->with(array('gender' => function ($query) {
    ...
    public function userLogAjax(Request $request)
    {
        ...
        $user_logs = AramiscUserLog::select('aramisc_user_logs.*')->where('academic_id', getAcademicId())
            ->where('aramisc_user_logs.school_id', Auth::user()->school_id)
            ->orderBy('id', 'desc')
    ...
    public function leaveDefineList()
    {
        ...
        try{
            $leave_defines = AramiscLeaveDefine::with('role', 'user','leaveType')->where('academic_id', getAcademicId())->select('aramisc_leave_defines.*');
        ...
        if($request->ajax()){
            ...
            ->addColumn('invoice', function($row){
                if(moduleStatusCheck('University')){
                    ...
                    return aramisc_fees_invoice($row->installmentPayment->invoice_no ,$invoice_setting);
                }else{
    ...
    public function ajaxExpenseList(Request $request){
        ...
        public function bookListAjax(Request $request){
            if($request->ajax()){
                $books = AramiscBook::leftjoin('aramisc_subjects', 'aramisc_books.book_subject_id', '=', 'aramisc_subjects.id')
