...
class AramiscHolidayController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try{
            $holidays = AramiscHoliday::where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->orderby('id','DESC')->get();
        ...
        try{
            // $editData = AramiscHoliday::find($id);
             if (checkAdmin()) {
                ...
                $editData = AramiscHoliday::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
            }
        ...
        try{
            ...
            if ($request->file('upload_file_name') != "") {
                // $eventFile = AramiscHoliday::find($id);
                 if (checkAdmin()) {
                    ...
                    $eventFile = AramiscHoliday::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
                }
            ...
            // $holidays = AramiscHoliday::find($id);
               if (checkAdmin()) {
                    ...
                    $holidays = AramiscHoliday::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
                }
            $holidays->holiday_title = $request->holiday_title;
            $holidays->details = $request->details;
            $holidays->from_date = date('Y-m-d', strtotime($request->from_date));
