...
class AramiscApiController extends Controller
{
    ...
    {
        try {
            $classes = SmClass::withoutGlobalScope(StatusAcademicSchoolScope::class)->where('active_status', 1)->where('academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($school_id))->where('school_id',$school_id)->get();
            $student_list = DB::table('sm_students')
                ->join('sm_classes', 'sm_students.class_id', '=', 'sm_classes.id')
                ->join('sm_sections', 'sm_students.section_id', '=', 'sm_sections.id')
                ->where('sm_students.academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($school_id))
    ...
    public function saas_class_index(Request $request, $school_id)
    {
        try {
            ...
            $classes = SmClass::withoutGlobalScope(StatusAcademicSchoolScope::class)->where('academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($school_id))->where('active_status', '=', 1)->where('school_id',$school_id)->get();
    ...
    public function studentSubjectApi(Request $request, $id, $record_id)
    {
        ...
        $assignSubjects = DB::table('sm_assign_subjects')
            ...
            ->where('sm_assign_subjects.academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($request->user()->school_id))->get();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
    ...
    public function saas_studentSubjectApi(Request $request,$school_id, $id, $record_id)
    {
        try{
            ...
            $assignSubjects = DB::table('sm_assign_subjects')
                ...
                ->where('sm_assign_subjects.class_id', '=', @$record->class_id)
                ->where('sm_assign_subjects.section_id', '=', @$record->section_id)
                ->where('sm_assign_subjects.academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($request->user()->school_id))->where('sm_assign_subjects.school_id',$school_id)->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
                $data['student_subjects'] = $assignSubjects->map(function($v){
                    $v->id = (int) $v->id;
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            if ($teacher->role_id==1 || $teacher->role_id==5) {
                ...
                $teacher_classes = DB::table('sm_assign_subjects')
                    ...
                    ->select('class_id', 'class_name')
                    ->where('teacher_id', $teacher_id)
                    ->where('sm_classes.academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($school_id))
                    ->get();
            }
            $data['teacher_classes'] = $teacher_classes->toArray();
            return ApiBaseMethod::sendResponse($data, null);
