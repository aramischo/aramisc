...
class AramiscAcademicsController extends Controller
{
    ...
    function universityClassReport($request){
            ...
            try {
                ...
                if ($request->section != "") {
                    $sectionInfo = AramiscSection::where('un_academic_id', getAcademicId())->where('id', $request->un_section_id)->where('school_id', Auth::user()->school_id)->first();
                } else {
                    $sectionInfo = '';
                }
                
                $student_ids_from_record = AramiscStudentReportController::SemesterLabelSectionStudent($request);
                $students = AramiscStudent::query()->whereIn('id', $student_ids_from_record)->where('school_id', auth()->user()->school_id);
                ...
                $fees_masters = AramiscFeesMaster::whereIn('id', $fees_master_ids)->pluck('fees_type_id')->toArray();
    
                $total_collection = AramiscFeesPayment::where('active_status',1)->where('academic_id', getAcademicId())->whereIn('student_id', $student_ids)->whereIn('fees_type_id', $fees_masters)->sum('amount');
                $total_assign = 0;
                $total_due = 0;
                $applied_discount = 0;
                foreach ($students as $student) {
                    $fees_assigns = AramiscFeesAssign::where("student_id", $student->id)->get();
                    $fees_dues_master_ids=[];
                    foreach ($fees_assigns as $fees_assign) {
                        $fees_master = AramiscFeesMaster::where('academic_id', getAcademicId())->where('id', $fees_assign->fees_master_id)->first();
    ...
    if (moduleStatusCheck('University')) {
        return $this->universityClassReport($request);
            ...
            try {
                ...
                $student_ids_from_record = AramiscStudentReportController::classSectionStudent($request);
                $students = AramiscStudent::query()->whereIn('id', $student_ids_from_record)->where('school_id', auth()->user()->school_id);
                ...
                $fees_assigns = AramiscFeesAssign::whereIn("student_id", $student_ids)->get();
                $fees_master_ids=$fees_assigns->pluck('fees_master_id')->toArray();
