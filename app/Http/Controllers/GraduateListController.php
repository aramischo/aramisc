...
class GraduateListController extends Controller
{
    ...
    {
        $graduates = Graduate::where('school_id',auth()->user()->school_id)->with('student','section','aramiscClass','unFaculty','unDepartment')->get();
            ...
            return DataTables::of($records)
                ...
                ->filterColumn('full_name', function ($query, $keyword) {
                    $query->whereHas('student', function ($q) use ($keyword) {
                        ...
                        ->orWhereHas('aramiscClass', function ($q) use ($keyword) {
                ...
                ->addColumn('class_sec', function ($row) {
                    ...
                    if (moduleStatusCheck('University') && $row->unFaculty && $row->unDepartment) {
                        ...
                        if ($row->student && $row->student->studentRecords) {
                            ...
                            $class_sec = $uniqueClassSections->count() > 1 ? $uniqueClassSections->toArray() : [$row->aramiscClass->class_name . '(' . $row->section->section_name . ')'];
                        } else {
                            $class_sec[] = $row->aramiscClass->class_name . '(' . $row->section->section_name . ')';
                        }
                    }
