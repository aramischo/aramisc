...
class AramiscItemCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try{
            $itemCategories = AramiscItemCategory::where('school_id',Auth::user()->school_id)->orderby('id','DESC')->get();
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = AramiscItemCategory::where('school_id', Auth::user()->school_id)->where('category_name', $request->category_name)->first();
        if ($is_duplicate) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
         ...
         // school wise uquine validation
         $is_duplicate = AramiscItemCategory::where('school_id', Auth::user()->school_id)->where('category_name', $request->category_name)->where('id', '!=', $request->id)->first();
         if ($is_duplicate) {
             Toastr::error('Duplicate name found!', 'Failed');
             return redirect()->back()->withErrors($validator)->withInput();
         }
        ...
        try {
            if ($tables==null) {
                if (checkAdmin()) {
                    ...
                    $result = AramiscItemCategory::where('id',$id)->where('school_id',Auth::user()->school_id)->delete();
                }
