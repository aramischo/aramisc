...
class AramiscSectionController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       $academic_year=AramiscAcademicYear::where('school_id',Auth::user()->school_id)->where('id', getAcademicId())->first();
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = AramiscSection::where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->where('section_name', $request->name)->first();
        if ($is_duplicate) {
        ...
        try {
            // $section = AramiscSection::find($id);
             if (checkAdmin()) {
                ...
                $section = AramiscSection::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
            }
            $sections = AramiscSection::where('active_status', '=', 1)->orderBy('id', 'desc')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
    ...
    public function update(Request $request)
    {
        ...
        $is_duplicate = AramiscSection::where('school_id', Auth::user()->school_id)->where('academic_id', getAcademicId())->where('section_name', $request->name)->where('id','!=', $request->id)->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate section name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
        ...
        try {
            ...
            if (checkAdmin()) {
                ...
                $section = AramiscSection::where('id',$request->id)->where('school_id',Auth::user()->school_id)->first();
            }
            $section->section_name = $request->name;
            $result = $section->save();
            ...
            $tables = SmClassSection::where('section_id',$id)->first();
                if ($tables == null) {
                      if (checkAdmin()) {
                        ...
                        $delete_query = AramiscSection::where('id',$request->id)->where('school_id',Auth::user()->school_id)->delete();
                    }
                        if ($delete_query) {
                            Toastr::success('Operation successful', 'Success');
                            return redirect('section');
