...
class ApiAramiscStudentAttendanceController extends Controller
{
    public function studentAttendanceCheck(Request $request)
    {
        ...
        $students = StudentRecord::with(['studentDetail' => function ($q) {
            ...
            ->where('academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($request->user()->school_id))
            ->where('school_id', $request->user()->school_id)
            ->where('is_promote', 0)
            ->get();

        $studentAttendances = AramiscStudentAttendance::whereIn('student_id', $students->pluck('student_id')->unique())
        ...
        if (count($studentAttendances) == 0) {
            ...
            foreach ($students as $record) {
                $studentAttendanceFirst = AramiscStudentAttendance::where('student_id', $record->student_id)
                    ->where('attendance_date', date('Y-m-d', strtotime($request->date)))
    ...
    public function saas_studentAttendanceCheck(Request $request, $school_id)
    {
        ...
        $students = StudentRecord::with(['studentDetail' => function ($q) {
            ...
            ->where('is_promote', 0)
            ->get();
        $studentAttendance = AramiscStudentAttendance::whereIn('student_id', $students->pluck('student_id')->unique())
            ->where('attendance_date', date('Y-m-d', strtotime($request->date)))
            ->orderby('student_id', 'ASC')
            ->where('academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($request->user()->school_id))
    ...
    public function studentAttendanceStoreFirst(Request $request)
    {
        ...
        $students = AramiscStudent::whereIn('id', $student_ids)->select('id')->get();
        $attendance = AramiscStudentAttendance::where('student_id', $request->id)->where('student_record_id', $request->record_id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->first();
        if (empty($attendance)) {
            foreach ($students as $student) {
                $attendance = AramiscStudentAttendance::where('student_id', $student->id)->where('student_record_id', $request->record_id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->first();
                if ($attendance != "") {
                    $attendance->delete();
                } else {
    ...
    public function saas_studentAttendanceStoreFirst(Request $request, $school_id)
    {
        ...
        $student_ids = studentRecords($request, null, null)->get()->pluck('student_id')->unique();
        $students = AramiscStudent::whereIn('id', $student_ids)->where('school_id', $school_id)->select('id')->get();

        $attendance = AramiscStudentAttendance::where('student_id', $request->id)->where('student_record_id', $request->record_id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('school_id', $school_id)->first();
        if (empty($attendance)) {
            foreach ($students as $student) {
                $attendance = AramiscStudentAttendance::where('student_id', $student->id)->where('student_record_id', $request->record_id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('school_id', $school_id)->first();
                if ($attendance != "") {
                    $attendance->delete();
                } else {
                    $attendance = new AramiscStudentAttendance();
                    ...
                    $attendance->section_id = $request->section;
                    $attendance->attendance_date = date('Y-m-d', strtotime($request->date));
                    $attendance->academic_id = AramiscAcademicYear::API_ACADEMIC_YEAR($school_id);
                    $attendance->save();
                }
            }
        }
    ...
    public function studentAttendanceStoreSecond(Request $request)
    {
        ...
        $student_ids = studentRecords($request, null, null)->get()->pluck('student_id')->unique();
        $students = AramiscStudent::whereIn('id', $student_ids)->select('id')->get();
        $attendance = AramiscStudentAttendance::where('student_id', $request->id)->where('student_record_id', $request->record_id)
            ->where('attendance_date', date('Y-m-d', strtotime($request->date)))
            ->first();
        // if (empty($attendance)) {
        //     foreach ($students as $student) {
        //         $attendance = AramiscStudentAttendance::where('student_id', $student->id)->where('student_record_id', $request->record_id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('school_id', auth()->user()->school_id)->first();
        //         if ($attendance != "") {
        //             $attendance->delete();
        //         }
    ...
    public function saas_studentAttendanceStoreSecond(Request $request, $school_id)
    {
        ...
        $student_ids = studentRecords($request, null, null)->get()->pluck('student_id')->unique();
        $students = AramiscStudent::whereIn('id', $student_ids)->select('id')->where('school_id', $school_id)->get();
        $attendance = AramiscStudentAttendance::where('student_id', $request->id)->where('student_record_id', $request->record_id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('school_id', $school_id)->first();
        // if (empty($attendance)) {
        //     foreach ($students as $student) {
        //         $attendance = AramiscStudentAttendance::where('student_id', $student->id)->where('student_record_id', $request->record_id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('school_id', $school_id)->first();
        ...
        //     }
        // }
        $attendance = AramiscStudentAttendance::where('student_id', $request->id)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('school_id', $school_id)->first();
        if ($attendance != "") {
            $attendance->delete();
        }
        $attendance = new AramiscStudentAttendance();
    ...
    public function studentSearch(Request $request)
    {
        ...
        try {
            ...
            $attendance_type = "";
            foreach ($students as $student) {
                $attendance = AramiscStudentAttendance::where('student_id', $student->id)->where('attendance_date', date('Y-m-d', strtotime($request->attendance_date)))->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->first();
    ...
    public function studentAttendanceStore(Request $request)
    {
        ...
        try {
            foreach ($request->id as $student) {
                $attendance = AramiscStudentAttendance::where('student_id', $student)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->first();
    ...
    public function studentAttendanceReportSearch(Request $request)
    {
        ...
        try {
            ...
            $attendances = [];
            foreach ($students as $student) {
                $attendance = AramiscStudentAttendance::where('student_id', $student->id)->where('attendance_date', 'like', $request->year . '-' . $request->month . '%')->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->get();
    ...
    public function saas_studentAttendanceReportSearch(Request $request, $school_id)
    {
        ...
        try {
            ...
            $attendances = [];
            foreach ($students as $student) {
                $attendance = AramiscStudentAttendance::where('student_id', $student->id)->where('attendance_date', 'like', $request->year . '-' . $request->month . '%')->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('school_id', $school_id)->get();
                if (count($attendance) != 0) {
                    $attendances[] = $attendance;
                }
            }
    ...
    public function studentStoreAttendanceAllApi(Request $request)
    {
        try {
            ...
            foreach ($request->attendance as $record_id => $student) {
                ...
                $attendance->academic_id = AramiscAcademicYear::API_ACADEMIC_YEAR($request->user()->school_id);
