...
class StudentFeesController extends Controller
{
    ...
    {
        $paymentMethods = AramiscPaymentMethhod::withoutGlobalScope(ActiveStatusSchoolScope::class)
    ...
    public function studentFeesPaymentStore(Request $request)
    {
        ...
        if ($request->payment_method == "Wallet") {
            ...
            if (moduleStatusCheck('University')) {
                $storeTransaction->un_academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            } else {
                $storeTransaction->academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            }
            ...
            foreach ($request->fees_type as $key => $type) {
                ...
                if ($request->paid_amount[$key] > 0) {
                    ...
                    if (moduleStatusCheck('University')) {
                        $storeTransactionChield->un_academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
                    } else {
                        $storeTransactionChield->academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
                    }
            ...
            // Income
            $payment_method = AramiscPaymentMethhod::withoutGlobalScope(ActiveStatusSchoolScope::class)
            ...
            $add_income->academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            $add_income->save();
    ...
    public function directFeesGenerateModalChild(Request $request)
    {
        ...
        foreach ($discounts as $fees_discount) {
            $fees_payment = AramiscFeesPayment::where('record_id', $record_id)->where('active_status', 1)->select('fees_discount_id')->where('fees_discount_id', $fees_discount->id)->where('school_id', auth()->user()->school_id)->first();
            if (isset($fees_payment->fees_discount_id)) {
                $applied_discount[] = $fees_payment->fees_discount_id;
            }
        }

        $data1['bank_info'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Bank')->where('school_id', auth()->user()->school_id)->first();
        $data1['cheque_info'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Cheque')->where('school_id', auth()->user()->school_id)->first();
        $data['bank_info'] = AramiscPaymentMethhod::select('id', 'method', 'type')->where('method', 'Bank')->where('school_id', auth()->user()->school_id)->first();
        $data1['cheque_info'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Cheque')->where('school_id', auth()->user()->school_id)->first();
        $data['cheque_info'] = AramiscPaymentMethhod::select('id', 'method', 'type')->where('method', 'Cheque')->where('school_id', auth()->user()->school_id)->first();
        $data1['PayPal'] = AramiscPaymentMethhod::where('method', 'PayPal')
            ->where('school_id', auth()->user()->school_id)
            ->first('active_status');
        $data1['Stripe'] = AramiscPaymentMethhod::where('method', 'Stripe')
            ->where('school_id', auth()->user()->school_id)
            ->first('active_status');
        $data1['Paystack'] = AramiscPaymentMethhod::where('method', 'Paystack')
            ->where('school_id', auth()->user()->school_id)
            ->first('active_status');
        if (moduleStatusCheck('CcAveune')) {
            $data1['CcAveune'] = AramiscPaymentMethhod::where('method', 'CcAveune')
                ->where('school_id', auth()->user()->school_id)
                ->first('active_status');
        }

        if (moduleStatusCheck('PhonePay')) {
            $data1['PhonePe'] = AramiscPaymentMethhod::where('method', 'PhonePay')
                ->where('school_id', auth()->user()->school_id)
                ->first('active_status');
        }

        $data['PayPal'] = AramiscPaymentGatewaySetting::where('gateway_name', 'PayPal')->where('school_id', auth()->user()->school_id)->first();
        $data['Stripe'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Stripe')->where('school_id', auth()->user()->school_id)->first();
        $data['Paystack'] = AramiscPaymentGatewaySetting::where('gateway_name', 'Paystack')->where('school_id', auth()->user()->school_id)->first();
        $data['PhonePe'] = AramiscPaymentGatewaySetting::where('gateway_name', 'PhonePay')->where('school_id', auth()->user()->school_id)->first();
    ...
    public function childBankSlipStore(Request $request)
    {
        if ($request->payment_mode == 'PayPal' || $request->payment_mode == 'Stripe' || $request->payment_mode == 'Paystack' || $request->payment_mode == 'CcAveune' || $request->payment_mode == 'PhonePay') {
            if (directFees()) {
                ...
                $data = [];
                $serviceCharge = 0;
                $gateway_setting = AramiscPaymentGatewaySetting::where('gateway_name', $request->payment_mode)->where('school_id', auth()->user()->school_id)->first();
                if ($gateway_setting) {
        ...
        if (moduleStatusCheck('University')) {
            $payment->un_academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            ...
            $new_subPayment->un_academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            $new_subPayment->created_by = auth()->user()->id;
            $new_subPayment->updated_by =  auth()->user()->id;
            $new_subPayment->school_id = auth()->user()->school_id;
            $new_subPayment->balance_amount = ($payable_amount - ($sub_payment + $request->amount));
    ...
    {
        if (checkAdmin()) {
            $payment = AramiscFeesPayment::where('school_id', auth()->user()->school_id)->where('id', $request->fees_payment_id)->first();
        } else {
            $payment = AramiscFeesPayment::where('active_status', 1)
                ->where('id', $request->fees_payment_id)
                ->where('school_id', auth()->user()->school_id)->first();
        }

        $group = $request->group;
        $student = AramiscStudent::withoutGlobalScope(SchoolScope::class)->where('id', $payment->student_id)->first();
