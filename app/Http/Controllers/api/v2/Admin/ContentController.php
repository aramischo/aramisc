...
class ContentController extends Controller
{
    ...
    public function uploadContents()
    {
        if (!teacherAccess()) {
            ...
            })->whereNullLms()->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())
                ->where('school_id', auth()->user()->school_id)
                ->get()
                ->map(function ($row) {
                    if ($row->available_for_admin == 1) {
    ...
    public function storeContent(Request $request)
    {
                ...
                if ($value == 'admin') {
                    ...
                    foreach ($roles as $role) {
                        ...
                        foreach ($staffs as $staff) {
                            ...
                            if (moduleStatusCheck('University')) {
                                $notification->un_academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
                            } else {
        ...
        $data = AramiscTeacherUploadContent::where('id', $uploadContents->id)
    ...
    {
        $request->validate([
            'content_id' => 'required|exists:aramisc_teacher_upload_contents,id'
        ], [
            'content_id.exists' => 'Invalid content'
        ]);

        $id =  $request->content_id;
        $uploadContent = AramiscTeacherUploadContent::withoutGlobalScope(GlobalAcademicScope::class)->where('id', $id)->where('school_id', auth()->user()->school_id)->first();
