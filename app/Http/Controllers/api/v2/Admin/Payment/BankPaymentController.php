...
class BankPaymentController extends Controller
{
    public function paymentList(Request $request)
    {
        ...
        $feesPayments->whereBetween('created_at', [$request->start_date, $request->end_date]);

        $data = BankPaymentListResourse::collection($feesPayments->get())->groupBy('paid_status');
    ...
    public function changePaymentStatus(Request $request)
    {
        ...
        switch ($request->change_status) {
            case 'approve':
                ...
                $student = AramiscStudent::withoutGlobalScope(SchoolScope::class)->with('parents')
                    ->where('school_id', auth()->user()->school_id)
