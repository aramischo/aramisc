...
class AttendanceController extends Controller
{
    ...
    public function sectionList(Request $request)
    {
        if ($request->parent) {
            ...
            foreach ($sectionIds as $sectionId) {
                $promote_sections[] = AramiscSection::withoutGlobalScope(StatusAcademicSchoolScope::class)
                    ->where('school_id', auth()->user()->school_id)
    ...
    public function studentSearch(Request $request)
    {
        ...
        $holiday = AramiscStudentAttendance::withoutGlobalScope(AcademicSchoolScope::class)
            ->where('class_id', $request->class_id)
    ...
    public function studentAttendanceHoliday(Request $request)
    {
        ...
        if ($request->purpose == "mark" && $holiday) {
            ...
            foreach ($studentRecords as $record) {
                $attendance = AramiscStudentAttendance::withoutGlobalScope(AcademicSchoolScope::class)
    ...
    public function search(Request $request)
    {
        ...
        $data['section_name'] = AramiscSection::withoutGlobalScopes([StatusAcademicSchoolScope::class])
    ...
    {
        $request->validate([
            'class' => 'required|exists:aramisc_class_sections,class_id',
            'section' => 'required|exists:aramisc_class_sections,section_id'
        ]);
    ...
    public function studentAttendanceReportSearch(Request $request)
    {
        ...
        $attendances = AramiscStudentAttendance::withoutGlobalScope(AcademicSchoolScope::class)->where('student_id', $record->student_id)
            ->whereYear('attendance_date', $year)
            ->whereMonth('attendance_date', $month)
            ->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('school_id', auth()->user()->school_id)
            ->where('student_record_id', $record->id)
    ...
    public function studentAttendanceStore(Request $request)
    {
        foreach ($request->get('record_id', []) as $key => $record) {
            $attendance = AramiscStudentAttendance::withoutGlobalScope(AcademicSchoolScope::class)
                ...
                ->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->first();
    ...
    public function subjectHolidayStore(Request $request)
    {
        ...
        $active_students = AramiscStudent::withoutGlobalScope(SchoolScope::class)->where('active_status', 1)
            ->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())
            ->where('school_id', auth()->user()->school_id)
