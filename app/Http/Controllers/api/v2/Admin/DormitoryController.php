...
class DormitoryController extends Controller
{
    public function store(Request $request)
    {
        $this->validate($request, [
            'dormitory_name' => "required|max:200|unique:aramisc_dormitory_lists",
            'type' => "required",
            'address' => "required",
            'intake' => "required",
        ...
        if (moduleStatusCheck('University')) {
            $dormitory_list->un_academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
        } else {
    ...
    public function dormitoryRoomStore(Request $request)
    {
        ...
        if (moduleStatusCheck('University')) {
            $room_list->un_academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
        } else {
    ...
    {
        $data = AramiscRoomType::withoutGlobalScope(ActiveStatusSchoolScope::class)
            ->where('school_id', auth()->user()->school_id)
            ->select('id', 'type')
            ->get();
    ...
    {
        $data = AramiscDormitoryList::withoutGlobalScope(ActiveStatusSchoolScope::class)
        ->where('school_id', auth()->user()->school_id)
