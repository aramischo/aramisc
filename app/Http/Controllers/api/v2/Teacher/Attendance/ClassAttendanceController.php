...
class ClassAttendanceController extends Controller
{
            ...
            $data = AramiscAssignSubject::withoutGlobalScope(StatusAcademicSchoolScope::class)
                ...
                ->get()->map(function ($class) {
                    return [
                        ...
                        'class_name'    => (string)$class->class->class_name,
                    ];
                });
        }
    ...
    public function sections(Request $request)
    {
        if (teacherAccess()) {
            ...
            foreach ($sectionIds as $sectionId) {
                $data[] = AramiscSection::withoutGlobalScope(StatusAcademicSchoolScope::class)
    ...
    public function students(Request $request)
    {
        ...
        /* $attendancee = AramiscStudentAttendance::withoutGlobalScope(AcademicSchoolScope::class)
    ...
    public function storeAttendance(Request $request)
    {
        if (teacherAccess()) {
            foreach ($request->get('record_id', []) as $key => $record) {
                $attendance = AramiscStudentAttendance::withoutGlobalScope(AcademicSchoolScope::class)->where('student_id', $request->student_id[$key])
    ...
    {
        $studentRecords = StudentRecord::where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())
            ->where('school_id', auth()->user()->school_id)
            ->where('class_id', $request->class_id)
            ->where('section_id', $request->section_id)
            ->get();
