...
class ContentController extends Controller
{
    ...
    {
        $uploadContents = AramiscTeacherUploadContent::withoutGlobalScope(GlobalAcademicScope::class)
    ...
    {
        $request->validate([
            'content_id' => ['required', Rule::exists('aramisc_teacher_upload_contents', 'id')->where('school_id', auth()->user()->school_id)]
        ]);

        $id =  $request->content_id;

        $uploadContent = AramiscTeacherUploadContent::withoutGlobalScope(GlobalAcademicScope::class)
            ->where('id', $id)
            ->where('school_id', auth()->user()->school_id)
            ->first();
