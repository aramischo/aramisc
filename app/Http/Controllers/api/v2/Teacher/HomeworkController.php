...
class HomeworkController extends Controller
{
    ...
    public function search(Request $request)
    {
        $this->validate($request, [
            'class_id' => ['required', Rule::exists('aramisc_homeworks', 'class_id')->where('school_id', auth()->user()->school_id)],
            'subject_id' => ['nullable', 'required_with:section_id', Rule::exists('aramisc_homeworks', 'subject_id')->where('school_id', auth()->user()->school_id)],
            'section_id' => ['nullable', Rule::exists('aramisc_homeworks', 'section_id')->where('school_id', auth()->user()->school_id)],
        ]);
    ...
    public function evaluationHomework(Request $request)
    {
        ...
        $students = AramiscStudent::withoutGlobalScope(SchoolScope::class)
            ->with(['homeworkContents', 'homeworks' => function ($q) use ($request) {
    ...
    $teacher_info = AramiscStaff::withoutGlobalScope(ActiveStatusSchoolScope::class)
        ...
        if ($teacher_info) {
            $data = $teacher_info->classes()
                ->select('aramisc_classes.id', 'aramisc_classes.class_name')
                ->get();
        }
        } else {
    ...
    public function addHomeworkDropdownListForSection(Request $request)
    {
        ...
        foreach ($sectionIds as $sectionId) {
            if ($request->globalType) {
                $promote_sections[] = AramiscSection::withoutGlobalScopes([StatusAcademicSchoolScope::class, GlobalAcademicScope::class])
    ...
    private static function classSectionStudent($request)
    {
        $student_ids = StudentRecord::when($request->academic_year, function ($query) use ($request) {
            ...
            ->when(!$request->academic_year, function ($query) {
                $query->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR());
