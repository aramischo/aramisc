...
use App\Http\Resources\AramiscStudentResourse;
...
use App\Http\Resources\AramiscStudentTransportResourse;
...
class AuthenticationController extends Controller
{
    public function login(Request $request)
    {
        ...
        if ($user) {
            ...
            if (Hash::check($request->password, $user->password)) {
                $data = [];
                $data['unread_notifications'] = (int)AramiscNotification::where('user_id', $user->id)->where('is_read', 0)->count();
                $data['user'] = [
                    'id' => (int)$user->id,
    ...
    public function DemoUser(Request $request, $role_id)
    {
        ...
        if ($user) {
            ...
            $data['unread_notifications'] = AramiscNotification::where('user_id', $user->id)->where('is_read', 0)->count();
            $data['user'] = [
                'id' => (int)$user->id,
                'full_name' => (string)$user->full_name,
                'phone_number' => (string)$user->phone_number,
    ...
    {
        $data['student_detail'] = AramiscStudent::with(['bloodGroup' => function ($q1) {
        ...
        $data['driver'] = AramiscVehicle::select('aramisc_staffs.full_name', 'aramisc_staffs.mobile')->where('aramisc_vehicles.id', '=', $data['student_detail']->vechile_id)
            ->join('aramisc_staffs', 'aramisc_staffs.id', '=', 'aramisc_vehicles.driver_id')
            ->first();

        $data['show_permission'] = AramiscStudentRegistrationField::where('school_id', app('school')->id)->pluck('is_show', 'field_name');
    ...
    {
        $data['student_detail'] = AramiscStudent::withoutGlobalScope(SchoolScope::class)
        ...
        $data['edit_permission'] = AramiscStudentRegistrationField::select('field_name', 'student_edit')->where('school_id', app('school')->id)->get();
    ...
    public function studentProfileUpdate(Request $request)
    {
        ...
        $data['profilePersonal'] = AramiscStudent::withoutGlobalScope(SchoolScope::class)
            ->where('school_id', auth()->user()->school_id)
    ...
    public function studentProfileImgUpdate(Request $request)
    {
        ...
        $proifle_details = AramiscStudent::withoutGlobalScope(SchoolScope::class)
            ->with(['bloodGroup', 'religion', 'defaultClass', 'defaultClass.class', 'defaultClass.section'])
            ->where('school_id', auth()->user()->school_id)
            ->where('user_id', $student->user_id)->first();
        $data['profilePersonal'] = new AramiscStudentResourse($proifle_details);
    ...
    {
        $students = AramiscStudent::withoutGlobalScope(SchoolScope::class)
            ->where('school_id', auth()->user()->school_id)
            ->where('user_id', auth()->user()->id)->first();
    ...
    public function profileTransport()
    {
        ...
        $data['show_permission'] = AramiscStudentRegistrationField::where('school_id', auth()->user()->school_id ?? app('school')->id)->pluck('is_show', 'field_name');
        if (!$data) {
            $response = [
                'success' => false,
                'data'    => null,
    ...
    {
        $data['profileOthers'] = AramiscStudent::withoutGlobalScope(SchoolScope::class)
            ->where('school_id', auth()->user()->school_id)
    ...
    public function profileDocuments()
    {
        ...
        $data['profileDocuments'] = AramiscStudentDocument::where('school_id', auth()->user()->school_id)
    ...
    public function studentRecord(Request $request)
    {
        ...
        $record = StudentRecord::with('class', 'section')
            ->where(['student_id' => $student->id, 'academic_id' => AramiscAcademicYear::API_ACADEMIC_YEAR($student->school_id)])
            ->where('school_id', auth()->user()->school_id)
            ->get();
    ...
    public function profileDocumentsStore(Request $request)
    {
        ...
        if ($request->file('photo') != "" && $request->title != "") {
            ...
            $document->academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            $document->save();
            $doc = [
                'id'    => (int)$document->id,
                'title' => (string)$document->title,
    ...
    {
        $request->validate([
            'document_id' => 'required|exists:aramisc_student_documents,id'
        ], [
            'document_id.exists' => 'The selected document is invalid.'
        ]);

        if (checkAdmin()) {
            $document = AramiscStudentDocument::find($request->document_id);
        } else {
            $document = AramiscStudentDocument::where('id', $request->document_id)
