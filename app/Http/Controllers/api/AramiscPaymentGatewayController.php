...
class AramiscPaymentGatewayController extends Controller
{
    public function dataSave(Request $request){
        ...
        $fees_payment->academic_id = AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
    ...
    public function successCallback(Request $request){
        ...
        if($request->payment_ref && $request->status){
            $fees_payment = AramiscFeesPayment::find($request->payment_ref);
            if( $fees_payment){
