...
class ApiAramiscClassRoutineController extends Controller
{
    public function classRoutineSearch(Request $request)
    {
        ...
        try {
            ...
            $aramisc_weekends = AramiscWeekend::with('classRoutine')->where('school_id', $school_id)
                ->orderBy('order', 'ASC')
            ...
            $teachers = AramiscStaff::where('role_id', 4)->where('school_id', $school_id)->get(['id', 'full_name', 'user_id', 'school_id']);

            if (!$class_id) {
                Session::put('session_day_id', null);
            }

            return response()->json(compact('classes', 'teachers', 'rooms', 'subjects', 'class_id', 'section_id', 'aramisc_weekends'));
        } catch (\Exception$e) {
    ...
    public function dayWiseClassRoutine(Request $request)
    {
        ...
        $school_id = auth()->user()->school_id;
        $class_routines = AramiscClassRoutineUpdate::where('day', $day_id)->where('class_id', $class_id)->where('section_id', $section_id)->orderBy('start_time', 'ASC')->where('academic_id', getAcademicId())->where('school_id', $school_id)->get();
        ...
        $rooms = AramiscClassRoom::where('active_status', 1)->where('capacity', '>=', $stds)
            ...
            ->get();
        $teachers = AramiscStaff::where('role_id', 4)->where('school_id', $school_id)->get(['id', 'full_name', 'user_id', 'school_id']);
        $aramisc_weekends = AramiscWeekend::where('school_id', $school_id)
            ->orderBy('order', 'ASC')
            ->where('active_status', 1)
            ->get(['id', 'name', 'order', 'is_weekend']);
        return response()->json(compact('day_id', 'class_routines', 'aramisc_weekends', 'subjects', 'rooms', 'teachers', 'section_id', 'class_id'));
    }
    ...
    public function studentClassRoutine(Request $request, $user_id, $record_id = null)
    {
        try {
            ...
            $class_routines = AramiscClassRoutineUpdate::withOutGlobalScope(StatusAcademicSchoolScope::class)->with('weekend', 'classRoom', 'subject', 'teacherDetail', 'class', 'section')->where('class_id', $class_id)->where('section_id', $section_id)
    ...
    public function teacherClassRoutine($user_id, $school_id = null)
    {
        try {
            ...
            $staff_detail = AramiscStaff::select('id', 'full_name', 'role_id')
                ->where('user_id', $user_id)
                ->first();
            ...
            $class_routines = AramiscClassRoutineUpdate::with('weekend', 'classRoom', 'subject', 'teacherDetail', 'class', 'section')->where('teacher_id', $teacher_id)->where('school_id', $school_id)->get()->map(function ($value) {
                return [
                    'id' => $value->id,
    ...
    public function classRoutineReportSearch(Request $request)
    {
        try {
            ...
            $class_routines = AramiscClassRoutineUpdate::with('weekend', 'classRoom', 'subject', 'teacherDetail', 'class', 'section')->where('class_id', $class_id)->where('section_id', $section_id)
                ->where('school_id', $school_id)->get()->map(function ($value) {
                return [
                    'id' => $value->id,
                    'day' => $value->weekend ? $value->weekend->name : '',
    ...
    {
        $teachers = AramiscStaff::where('role_id', 4)->where('school_id', auth()->user()->school_id)->get(['id', 'full_name', 'user_id', 'school_id']);
    ...
    public function sectionRoutine(Request $request, $user_id, $class, $section)
    {
        try {
            ...
            $staff_detail = AramiscStaff::select('id', 'full_name')
                ->where('user_id', $user_id)
                ->first();
            if ($staff_detail->role_id !=4) {
                return response()->json(['message'=>'You Are not teacher']);
            ...
            $class_routines = AramiscClassRoutineUpdate::with('weekend', 'classRoom', 'subject', 'teacherDetail', 'class', 'section')->where('teacher_id', $teacher_id)->where('class_id', $class)->where('section_id', $section)->where('school_id', $school_id)->get()->map(function ($value) {
                return [
                    'id' => $value->id,
                    'day' => $value->weekend ? $value->weekend->name : '',
                    'room' => $value->classRoom ? $value->classRoom->room_no : '',
