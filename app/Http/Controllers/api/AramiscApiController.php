...
class AramiscApiController extends Controller
{
    ...
    public function AttendanceSync(Request $request)
    {
        ...
        try {
            ...
            $attendance_setting = AramiscBioSetting::find(1);
            ...
            foreach ($lastest_data as $device_log) {
                ...
                if ($user_table) {
                    if ($user_table->role_id == 2) {
                        $aramisc_students = DB::table('aramisc_students')->where('user_id', $device_log->userid)->first();
                        $to_mobile = $aramisc_students->mobile;

                        if ($aramisc_students) {
                            DB::table('device_log')->where('userid', $device_log->userid)->update(array('role_id' => $aramisc_students->role_id, 'class_id' => $aramisc_students->class_id, 'section_id' => $aramisc_students->section_id, 'profile_id' => $aramisc_students->id));
                        }
                        $attendance = AramiscStudentAttendance::where('student_id', $aramisc_students->id)->where('attendance_date', date('Y-m-d', strtotime($device_log->checktime)))->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->first();
                        if ($attendance == "") {
                            $attendance = new AramiscStudentAttendance();
                        }
                        $attendance->student_id = $aramisc_students->id;
                        ...
                        } elseif ($attendance_start_time <= $d_start_time && $attendance_setting->late_time >= $d_start_time && isset($aramisc_students->id) && isset($sms_template->late_sms)) {
                            $attendance->attendance_type = 'LP';

                        } elseif ($attendance_setting->late_time <= $d_start_time && $attendance_setting->absent <= $d_start_time && isset($aramisc_students->id) && isset($sms_template->absent)) {
                            $attendance->attendance_type = 'A';

                        } elseif ($attendance_setting->early_checkout >= $d_start_time && $attendance_setting->end_time <= $d_start_time && isset($aramisc_students->id) && isset($sms_template->er_checkout)) {
                            $attendance->attendance_type = 'EL';

                        } elseif ($attendance_setting->end_time <= $d_start_time && isset($aramisc_students->id) && isset($sms_template->st_checkout)) {
                            $attendance->attendance_type = 'P';
                        ...
                        $aramisc_staff = AramiscStaff::where('user_id', $device_log->userid)->first();

                        if ($aramisc_staff) {
                            DB::table('device_log')->where('userid', $device_log->userid)->update(array('role_id' => $aramisc_staff->role_id, 'profile_id' => $aramisc_staff->id));
                        }
                        $attendance = AramiscStaffAttendence::where('staff_id', $aramisc_staff->id)->where('attendence_date', date('Y-m-d', strtotime($device_log->checktime)))->first();
                        if ($attendance == "") {
                            $attendance = new AramiscStaffAttendence();
                        }
                        $attendance->staff_id = $aramisc_staff->id;
                        $attendance_start_time = $attendance_setting->start_time;
                        $d_start_time = date('H:s', strtotime($device_log->checktime));
    ...
    public function library_member_store(Request $request)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            if ($request->member_type == "") {
                $validator = Validator::make($input, [
                    ...
                    'created_by' => "required",
                    'member_ud_id' => "required|unique:aramisc_library_members,member_ud_id",
                ]);
        ...
        if ($request->student != 0) {
            ...
            $isData = AramiscLibraryMember::where('student_staff_id', '=', $student_staff_id)->where('active_status', '=', 1)->first();
            if (!empty($isData)) {
                if (ApiBaseMethod::checkUrl($request->fullUrl())) {
        ...
        $isExist_staff_id = AramiscLibraryMember::where('student_staff_id', '=', $student_staff_id)->first();
        if (!empty($isExist_staff_id)) {
            $members = AramiscLibraryMember::where('student_staff_id', '=', $student_staff_id)->first();
    ...
    public function saas_library_member_store(Request $request)
    {
        ...
        if ($request->student != 0) {
            $student_staff_id = $request->student;
            $isData = AramiscLibraryMember::where('student_staff_id', '=', $student_staff_id)->where('active_status', '=', 1)->where('school_id', '=', $request->school_id)->first();
            if (!empty($isData)) {
                if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                    return ApiBaseMethod::sendError('This Member is already added in our library.');
                }
        ...
        $isExist_staff_id = AramiscLibraryMember::where('student_staff_id', '=', $student_staff_id)->where('school_id', '=', $request->school_id)->first();
        if (!empty($isExist_staff_id)) {
            $members = AramiscLibraryMember::where('student_staff_id', '=', $student_staff_id)->where('school_id', '=', $request->school_id)->first();
            $members->active_status = 1;
            $results = $members->update();
            return ApiBaseMethod::sendResponse(null, 'New Member has been added successfully');
        } else {
                ...
                if (Hash::check($request->password, $user->password)) {
                    ...
                    if ($role_id == 2) {
                        ...
                        $data['transport'] = DB::table('aramisc_students')
                            ...
                            ->where('aramisc_students.user_id', $user->id)
                            ->first();
                        $data['system_settings'] = DB::table('aramisc_general_settings')->get();
    ...
    public function fees_group_update(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
            'name' => "required|max:200|unique:aramisc_fees_groups,name," . $request->id,
    ...
    public function saas_class_index(Request $request, $school_id)
    {
        try {
            $sections = AramiscSection::where('active_status', '=', 1)->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('school_id', $school_id)->get();
            $classes = AramiscClass::where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('active_status', '=', 1)->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('school_id', $school_id)->get();
    ...
    public function class_Routine(Request $request, $id = null)
    {
        ...
        $aramisc_weekends = AramiscWeekend::where('school_id', $student_detail->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
        $class_times = AramiscClassTime::where('type', 'class')->where('academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->get();

        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
            $data['student_detail'] = $student_detail->toArray();

            $weekenD = AramiscWeekend::where('school_id', $student_detail->school_id)->get();
            foreach ($weekenD as $row) {
                $data[$row->name] = DB::table('aramisc_class_routine_updates')
                    ->orderBy('aramisc_class_times.start_time', 'asc')
                    ...
                    ])->where('aramisc_class_routine_updates.academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->get();
            }
    ...
    public function saas_class_Routine(Request $request, $school_id, $id = null)
    {
        ...
        $aramisc_weekends = AramiscWeekend::orderBy('order', 'ASC')->where('active_status', 1)->where('academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($school_id))->where('school_id', $student_detail->school_id)->get();
        $class_times = AramiscClassTime::where('type', 'class')->where('academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($school_id))->where('school_id', $school_id)->get();
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            foreach ($weekenD as $row) {
                $data[$row->name] = DB::table('aramisc_class_routine_updates')
                    ...
                    ->where([
                        ['aramisc_class_routine_updates.class_id', $class_id], ['aramisc_class_routine_updates.section_id', $section_id], ['aramisc_class_routine_updates.day', $row->id],
                    ])->where('aramisc_class_routine_updates.academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($school_id))
                    ->where('aramisc_class_routine_updates.school_id', $school_id)
                    ->get();
            }
    ...
    public function generalSettingsView(Request $request)
    {
        ...
        try {
            $editData = AramiscGeneralSettings::find(1);
    ...
    public function updateGeneralSettings(Request $request)
    {
        ...
        try {
            ...
            $currencies = AramiscCurrency::all();
    ...
    public function studentSubjectApi(Request $request, $id)
    {
        ...
        $assignSubjects = DB::table('aramisc_assign_subjects')
            ...
            ->where('aramisc_assign_subjects.academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->get();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
    ...
    public function saas_studentSubjectApi(Request $request, $school_id, $id)
    {
        ...
        $assignSubjects = DB::table('aramisc_assign_subjects')
            ...
            ->where('aramisc_assign_subjects.class_id', '=', @$student->class_id)
            ->where('aramisc_assign_subjects.section_id', '=', @$student->section_id)
            ->where('aramisc_assign_subjects.academic_id', AramiscAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('aramisc_assign_subjects.school_id', $school_id)->get();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
            $data['student_subjects'] = $assignSubjects->toArray();
            return ApiBaseMethod::sendResponse($data, null);
        ...
        try {
            ...
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                $online_exams = AramiscOnlineExam::where('aramisc_online_exams.active_status', 1)
                    ->where('aramisc_online_exams.academic_id', AramiscAcademicYear::API_ACADEMIC_YEAR($student->school_id))
                    ->join('aramisc_subjects', 'aramisc_subjects.id', '=', 'aramisc_online_exams.subject_id')

                    ->where('class_id', $student->class_id)
                    ->where('section_id', $student->section_id)
                    ->where('aramisc_online_exams.school_id', $student->school_id)
    ...
    public function saas_studentOnlineExamApi(Request $request, $school_id, $id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $online_exams = AramiscOnlineExam::where('aramisc_online_exams.status', '=', 1)
                ->join('aramisc_subjects', 'aramisc_online_exams.class_id', '=', 'aramisc_subjects.id')
                ->where('class_id', @$student->class_id)
                ->where('section_id', @$student->section_id)
                ->where('end_time', '>', $now)
                ->where('date', '=', $today)
                ->select('aramisc_online_exams.id as exam_id', 'aramisc_online_exams.title as exam_title', 'aramisc_subjects.subject_name', 'aramisc_online_exams.date', 'aramisc_online_exams.status as onlineExamStatus', 'aramisc_online_exams.status as onlineExamTakeStatus')
                ->where('aramisc_online_exams.school_id', $school_id)->get();
            $examStatus = '0 = Pending , 1 Published';
            $examTakenStatus = '0 = Take Exam , 1 = Alreday Submitted';
            $data['online_exams'] = $online_exams->toArray();
            $data['online_exams_status'] = $examStatus;
    ...
    public function examResultApi(Request $request, $id, $exam_id)
    {
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $exam_result = DB::table('aramisc_student_take_online_exams')
                ...
                ->where('aramisc_student_take_online_exams.student_id', $student->id)
                ->where('aramisc_student_take_online_exams.school_id', $student->school_id)
    ...
    public function saas_examResult_Api(Request $request, $school_id, $id, $exam_id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $exam_result = DB::table('aramisc_result_stores')
                ...
                ->where('aramisc_result_stores.student_id', '=', @$student->id)
                ->select('aramisc_exams.id', 'aramisc_exam_types.title as exam_name', 'aramisc_subjects.subject_name', 'aramisc_result_stores.total_marks as obtained_marks', 'aramisc_exams.exam_mark as total_marks', 'aramisc_result_stores.total_gpa_grade as grade')
                ->where('aramisc_result_stores.school_id', $school_id)->get();
    ...
    public function saas_childListApi(Request $request, $school_id, $id)
    {
        ...
        $parent = AramiscParent::where('user_id', $id)->where('school_id', $school_id)->first();
        $student_info = DB::table('aramisc_students')
            ->join('aramisc_classes', 'aramisc_classes.id', '=', 'aramisc_students.class_id')
            ->join('aramisc_sections', 'aramisc_sections.id', '=', 'aramisc_students.section_id')
            ...
            ->where('aramisc_students.school_id', $school_id)->get();
        ...
        try {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                $data['userDetails'] = DB::table('aramisc_students')->select('aramisc_students.*', 'aramisc_parents.*', 'aramisc_classes.*', 'aramisc_sections.*')
                    ...
                    ->join('aramisc_sections', 'aramisc_sections.id', '=', 'aramisc_students.section_id')
                    ->where('aramisc_students.id', $user->id)
                    ->where('aramisc_students.school_id', $school_id)->first();
    ...
    public function searchStudent(Request $request)
    {
        ...
        if (!empty($request->class) && !empty($request->section)) {
            $students = DB::table('aramisc_students')
                ->select('aramisc_students.id', 'student_photo', 'full_name', 'roll_no', 'class_name', 'section_name', 'user_id')
                ->join('aramisc_sections', 'aramisc_sections.id', '=', 'aramisc_students.section_id')
                ->join('aramisc_classes', 'aramisc_classes.id', '=', 'aramisc_students.class_id')
                ->where('aramisc_students.class_id', $request->class)
                ->where('aramisc_students.section_id', $request->section)
            ...
            $students = DB::table('aramisc_students')
                ...
                ->join('aramisc_sections', 'aramisc_sections.id', '=', 'aramisc_students.section_id')
                ->join('aramisc_classes', 'aramisc_classes.id', '=', 'aramisc_students.class_id')
                ->where('aramisc_students.full_name', 'like', '%' . $request->name . '%')
                ->get();
            $msg = "Student Found";
            ...
            $students = DB::table('aramisc_students')
                ...
                ->join('aramisc_sections', 'aramisc_sections.id', '=', 'aramisc_students.section_id')
                ->join('aramisc_classes', 'aramisc_classes.id', '=', 'aramisc_students.class_id')
                ->where('aramisc_students.roll_no', $request->roll_no)
                ->get();
            $msg = "Student Found";
        } else {
    ...
    public function myRoutine(Request $request, $id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
            $weekenD = AramiscWeekend::where('school_id', $teacher->school_id)->get();
            foreach ($weekenD as $row) {
                $data[$row->name] = DB::table('aramisc_class_routine_updates')
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            foreach ($weekenD as $row) {
                $data[$row->name] = DB::table('aramisc_class_routine_updates')
                    ...
                    ->where([
                        ['aramisc_class_routine_updates.teacher_id', @_id], ['aramisc_class_routine_updates.day', $row->id],
                    ])->where('aramisc_class_routine_updates.school_id', $school_id)->get();
            }
    ...
    public function sectionRoutine(Request $request, $id, $class, $section)
    {
        ...
        $aramisc_weekends = AramiscWeekend::where('school_id', $teacher->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
        $class_times = AramiscClassTime::where('type', 'class')->get();
    ...
    public function saas_teacherClassList(Request $request, $school_id)
    {
        ...
        $teacher = AramiscStaff::where('user_id', '=', $request->id)->first();
        $teacher_id = $teacher->id;
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
            if ($teacher->role_id == 1) {
    ...
    public function teacherMyAttendanceSearchAPI(Request $request, $id = null)
    {
        ...
        try {
            ...
            $attendances = AramiscStaffAttendence::where('staff_id', $teacher->id)
