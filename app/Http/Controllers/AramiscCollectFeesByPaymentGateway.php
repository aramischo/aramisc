...
class AramiscCollectFeesByPaymentGateway extends Controller
{
    ...
    public function collectFeesByGateway($amount, $student_id, $type)
    {
        try {
            ...
            $discounts = AramiscFeesAssignDiscount::where('student_id', $student_id)->get();
    ...
    public function getPaymentStatus(Request $request)
    {
        ...
        if (!is_null($paypal_fees_paymentId)) {
            $fees_payment = AramiscFeesPayment::find($paypal_fees_paymentId);
        }
    ...
    public function collectFeesStripe($amount, $student_id, $type)
    {
        try {
            ...
            $student_id = $student_id;
            $discounts = AramiscFeesAssignDiscount::where('student_id', $student_id)->get();
            $stripe_publisher_key = AramiscPaymentGatewaySetting::where('gateway_name', '=', 'Stripe')->first()->stripe_publisher_key;

            $applied_discount = AramiscFeesPayment::select('fees_discount_id')->whereIn('fees_discount_id', $discounts->pluck('id')->toArray())->pluck('fees_discount_id')->toArray();
    ...
    public function stripeStore(Request $request)
    {
        try {
            ...
            $stripeDetails = AramiscPaymentGatewaySetting::select('stripe_api_secret_key', 'stripe_publisher_key')->where('gateway_name', '=', 'Stripe')->first();
