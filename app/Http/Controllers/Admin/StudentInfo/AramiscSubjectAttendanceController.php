...
class AramiscSubjectAttendanceController extends Controller
{
    ...
    public function search(StudentSubjectWiseAttendancSearchRequest $request)
    {
        try {
            ...
            if (!moduleStatusCheck('University')) {
                ...
                $search_info['subject_name'] = AramiscSubject::find($request->subject_id)->subject_name;
            }
    ...
    {
        $active_students = AramiscStudent::where('active_status', 1)
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
        ...
        if ($request->purpose == "mark") {
            foreach ($students as $record) {
                ...
                $subject = AramiscSubject::find($request->subject_id);
                $subject_name = $subject->subject_name;
    ...
    public function subjectAttendanceReportSearch(StudentSubjectWiseAttendanceSearchRequest $request)
    {
        ...
        try {
            ...
            $assign_subjects = AramiscAssignSubject::where('class_id', $class_id)
            ...
            foreach ($student_records as $record) {
                $attendance = AramiscSubjectAttendance::with('student')->where('student_record_id', $record->id)
...

    {
        ...
        try {
            ...
            $types = AramiscStudentCategory::withoutGlobalScope(AcademicSchoolScope::class)->where('school_id', Auth::user()->school_id)->get();
...

    {
        try {
            ...
            $assign_subjects = AramiscAssignSubject::where('class_id', $class_id)->where('section_id', $section_id)->first();
            ...
            $classes = AramiscClass::get();
            $activeStudentIds = AramiscStudentAttendanceController::activeStudent()->pluck('id')->toArray();
            ...
            foreach ($students as $record) {
                ...
                $attendance = AramiscSubjectAttendance::where('aramisc_subject_attendances.student_id', $record->student_id)

                    //  ->join('student_records','student_records.student_id','=','aramisc_subject_attendances.student_id')

                    // // ->where('subject_id', $subject_id)

                    ->where('attendance_date', 'like', $year . '-' . $month . '%')
                    ->where('aramisc_subject_attendances.student_record_id', $record->id)
                    ->where('aramisc_subject_attendances.academic_id', getAcademicId())
                    ->where('aramisc_subject_attendances.school_id', Auth::user()->school_id)
    ...
    public function studentAttendanceReportPrint($class_id, $section_id, $month, $year)
    {
        try {
            ...
            $attendances = [];
            foreach ($students as $record) {
                $attendance = AramiscStudentAttendance::where('student_id', $record->student_id)->where('attendance_date', 'like', $year . '-' . $month . '%')->where('school_id', Auth::user()->school_id)
                    ->where('student_record_id', $record->id)
                    ->get();
