...
class AramiscStudentAdmissionController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $data = $this->loadData();
            $data['max_admission_id'] = AramiscStudent::where('school_id', Auth::user()->school_id)->max('admission_no');
            $data['max_roll_id'] = AramiscStudent::where('school_id', Auth::user()->school_id)->max('roll_no');
    ...
    public function store(AramiscStudentAdmissionRequest $request)
    {
                ...
                if ($user->role_id == 2) {
                    ...
                    if (!$studentRecord) {
                        ...
                        if (moduleStatusCheck('Lead') == true && $request->lead_id) {
                            ...
                            $registrationStudent = \Modules\ParentRegistration\Entities\AramiscStudentRegistration::find($request->parent_registration_student_id);
    ...
    public function edit(Request $request, $id)
    {
        try {
            $data = $this->loadData();
            $data['student'] = AramiscStudent::with('sections')->select('aramisc_students.*')->find($id);
            $data['siblings'] = AramiscStudent::where('parent_id', $data['student']->parent_id)->whereNotNull('parent_id')->where('id', '!=', $id)->get();
    ...
    public function update(AramiscStudentAdmissionRequest $request)
    {
        try {
    ...
    public function view(Request $request, $id, $type = null)
    {
        try {
            ...
            $siblings = AramiscStudent::where('parent_id', '!=', 0)->where('parent_id', $student_detail->parent_id)->where('id', '!=', $id)->status()->withoutGlobalScope(StatusAcademicSchoolScope::class)->get();
            ...
            $sessions = AramiscAcademicYear::get(['id', 'year', 'title']);
    ...
    {
        try {
            $student_settings = AramiscStudentRegistrationField::where('school_id', auth()->user()->school_id)->where('active_status', 1)->get()->filter(function ($field) {
    ...
    {
        $field = AramiscStudentRegistrationField::where('school_id', auth()->user()->school_id)
    ...
    public function checkExitStudent(Request $request)
    {
        try {
            ...
            $student = null;
            if ($email || $phone) {
                $x_student = AramiscStudent::query();
                if ($email && $phone) {
    ...
    {
        try {
            $academic_years = AramiscAcademicYear::where('school_id', $request->school_id)->get();
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        try {
                            ...
                            $academic_year = moduleStatusCheck('University')
                                ? UnAcademicYear::find($request->un_session_id) : AramiscAcademicYear::find($request->session);
