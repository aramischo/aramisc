...
class AramiscStudentAttendanceReportController extends Controller
{
    //
    ...
    public function print($class_id, $section_id, $month, $year)
    {
        ...
        try {
            ...
            $active_students = AramiscStudent::where('active_status', 1)->where('school_id', Auth::user()->school_id)->pluck('id')->toArray();
    ...
    public function unPrint($semester_id, $month, $year)
    {
        ...
        try {
            $current_day = date('d');
            $days = cal_days_in_month(CAL_GREGORIAN, $month, $year);
            $active_students = AramiscStudent::where('active_status', 1)->where('school_id', Auth::user()->school_id)->where('academic_id', getAcademicId())->pluck('id')->toArray();
            $students = DB::table('student_records')
