...
class AramiscIncomeHeadController extends Controller
{
    ...
    public function index(){
		...
		try{
			$income_heads = AramiscIncomeHead::where('school_id',Auth::user()->school_id)->get();
    ...
    public function store(Request $request){
		$request->validate([
			'income_head' => "required|unique:aramisc_income_heads,name",
		]);
		try{
			$income_head = new AramiscIncomeHead();
    ...
    public function edit($id){
		 ...
		 try{
			// $income_head = AramiscIncomeHead::find($id);
			 if (checkAdmin()) {
				$income_head = AramiscIncomeHead::find($id);
			}else{
				$income_head = AramiscIncomeHead::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
			}
			$income_heads = AramiscIncomeHead::where('school_id',Auth::user()->school_id)->get();
    ...
    public function update(Request $request){
    	$request->validate([
			'income_head' => "required|unique:aramisc_income_heads,name,".$request->id,
		]);
		try{
			$fees_discount = AramiscIncomeHead::find($request->id);
    ...
    public function delete($id){
		...
		try{
			$fees_discount = AramiscIncomeHead::destroy($id);
			if($fees_discount){
				Toastr::success('Operation successful', 'Success');
				return redirect()->back();
