...
class AramiscExpenseHeadController extends Controller
{
    ...
    public function index()
    {
        ...
        try{
            $expense_heads = AramiscExpenseHead::where('school_id',Auth::user()->school_id)->get();
    ...
    {
        $request->validate([
            'name' => "required|unique:aramisc_expense_heads,name",
        ]);


        try{
            $expense_head = new AramiscExpenseHead();
    ...
    public function show($id)
    {
        ...
        try{
            $expense_head = AramiscExpenseHead::find($id);
            $expense_heads = AramiscExpenseHead::where('school_id',Auth::user()->school_id)->get();
    ...
    {
        $request->validate([
            'name' => "required|unique:aramisc_expense_heads,name,".$request->id,
        ]);
        try{
            $expense_head = AramiscExpenseHead::find($request->id);
            $expense_head->name = $request->name;
            $expense_head->description = $request->description;
            $result = $expense_head->save();
            if($result){
    ...
    public function destroy($id)
    {
        ...
        try{
            $expense_head = AramiscExpenseHead::destroy($id);
            if($expense_head){
                Toastr::success('Operation successful', 'Success');
                return redirect('expense-head');
