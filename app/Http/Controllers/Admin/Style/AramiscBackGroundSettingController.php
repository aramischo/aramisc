...
class SmBackGroundSettingController extends Controller
{
    ...
    {
        try {
            $background_settings = AramiscBackgroundSetting::where('school_id', Auth::user()->school_id)->orderby('id', 'DESC')->get();
    ...
    public function store(SmBackGroundSettingRequest $request)
    {
        ...
        try {
            ...
            $background_setting = new AramiscBackgroundSetting();
    ...
    {
        try {
            $background = AramiscBackgroundSetting::find($id);
            if ($background->is_default == 1 && $background->title == "Login Background") {
                AramiscBackgroundSetting::where([['is_default', 1], ['title', 'Login Background']])->where('school_id', Auth::user()->school_id)->update(['is_default' => 0]);
                $result = AramiscBackgroundSetting::where('id', $id)->update(['is_default' => 1]);
            } else if ($background->is_default == 1 && $background->title == "Dashboard Background") {
                AramiscBackgroundSetting::where([['is_default', 1], ['title', 'Dashboard Background']])->where('school_id', Auth::user()->school_id)->update(['is_default' => 0]);
                $result = AramiscBackgroundSetting::where('id', $id)->update(['is_default' => 1]);
            ...
            if (moduleStatusCheck('Lead')==true) {
                if ($background->is_default == 1 && $background->title == "Lead Form Background") {
                    AramiscBackgroundSetting::where([['is_default', 1], ['title', 'Lead Form Background']])->where('school_id', Auth::user()->school_id)->update(['is_default' => 0]);
                    $result = AramiscBackgroundSetting::where('id', $id)->update(['is_default' => 1]);
    ...
    public function update(SmBackGroundSettingRequest $request)
    {
        ...
        try {
            ...
            $background_setting = AramiscBackgroundSetting::find(1);
    ...
    {
        try {
            $delBGS = AramiscBackgroundSetting::where('id', $id)->where('is_default', 1)->first();
            if (empty($delBGS)) {
                AramiscBackgroundSetting::find($id)->delete();
    ...
    {
        try {
            $color_styles = AramiscStyle::where('school_id', Auth::user()->school_id)->get();
    ...
    public function colorThemeSet($id)
    {
        try {
            ...
            AramiscStyle::where('is_active', 1)->where('school_id', Auth::user()->school_id)->update(['is_active' => 0]);
            $result = AramiscStyle::where('id', $id)->update(['is_active' => 1]);
            if ($result) {
                ...
                $active_style = AramiscStyle::where('school_id', 1)->where('is_active', 1)->first();
