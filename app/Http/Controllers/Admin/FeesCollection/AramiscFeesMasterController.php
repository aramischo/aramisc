...
use App\Http\Requests\Admin\FeesCollection\AramiscFeesMasterRequest;
...
class AramiscFeesMasterController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
           $fees_groups = AramiscFeesGroup::get();
            $fees_masters = AramiscFeesMaster::with('feesTypes', 'feesGroups')->get();
            $already_assigned = [];
    ...
    public function store(AramiscFeesMasterRequest $request)
    {
        ...
        try {
            ...
            $fees_type = AramiscFeesType::find($feesTypeId);
            
            $combination = AramiscFeesMaster::where('fees_group_id', $feesGroupId)
    ...
    private function installmentCreate($master_id, $payload){
        ...
        $master = AramiscFeesMaster::find($master_id);
        $feesType = AramiscFeesType::find($master->fees_type_id);
    ...
    public function update(AramiscFeesMasterRequest $request, $id)
    {
        try {
            ...
            $fees_master->save();
            if (moduleStatusCheck('University')) {
                $feesGroup = AramiscFeesGroup::find($request->fees_group_id);
    ...
    public function deleteGroup(Request $request)
    {
        try {
            ...
            try {
                if(directFees()){
                     $master = AramiscFeesMaster::find($request->id);
                    if($master){
                        ...
                        if(count($master_installments)){
                            foreach($master_installments as $installment){
                                ...
                                if((!is_null($assignrd_inst)) &&  count($assignrd_inst->payments) > 0){
                                    ...
                                    AramiscFeesAssign::where('fees_master_id',$request->id)->delete();
                                    AramiscFeesType::where('id',$master->fees_type_id)->delete();
                                    AramiscFeesGroup::where('id',$master->fees_group_id)->delete();
                                    Toastr::success('Operation successful', 'Success');
                                    return redirect()->back();
                                }
                            }
    ...
    public function feesAssignSearch(Request $request)
    {
        ...
        try {
            ...
            $fees_master_ids = AramiscFeesMaster::where('fees_group_id', $request->fees_group_id)->pluck('id')->toArray();

            $pre_assigned = AramiscFeesAssign::whereIn('record_id', $student_ids)
        ...
        try {
            if (moduleStatusCheck('University')) {
                ...
                if($request->fees_assign_all == 1)
                {
                    ...
                    $students = AramiscStudent::with(['feesAssign', 'feesPayment', 'feesAssignDiscount', 'forwardBalance', 'user', 'parents', 'parents.parent_user'])
    ...
    private function studentIds($request)
    {
        ...
        {
            $aramisc_student_ids = AramiscStudent::when($request->category, function($query) use($request){
        ...
        $students = $students->when($request->category || $request->group, function($query) use($aramisc_student_ids){
            $query->whereIn('student_id', $aramisc_student_ids);
        })
    ...
    {
        try{
            $assigned_fees = AramiscFeesAssign::get();
