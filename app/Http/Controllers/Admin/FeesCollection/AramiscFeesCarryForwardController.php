...
use App\Http\Requests\Admin\FeesCollection\AramiscFeesForwardSearchRequest;

class AramiscFeesCarryForwardController extends Controller
{
        ...
        try {
            ...
            if ($students->count() != 0) {
                foreach ($students as $student) {
                    $fees_balance = AramiscFeesCarryForward::where('student_id', $student->student_id)->count();
                }
                if ($fees_balance == 0) {
    ...
    public function feesForwardSearch(AramiscFeesForwardSearchRequest $request)
    {
    ...
    public function feesCarryForwardStore(Request $request)
    {
        try {
            foreach (gv($request, 'studentFeesInfo') as $studentInfo) {
                ...
                $deleteData = AramiscFeesCarryForward::where('student_id', gv($studentInfo, 'student_id'))->first();
                $studentBaseAmount = AramiscFeesCarryForward::where('student_id', gv($studentInfo, 'student_id'))->sum('balance');
                if($deleteData){
