...
class AramiscFeesDiscountController extends Controller
{
        ...
        try {
            ...
            foreach ($students as $key => $student) {
                $assigned_fees_type=AramiscFeesAssign::where('student_id',$student->student_id)
                        ...
                        ->where('aramisc_fees_assigns.school_id',Auth::user()->school_id)
                        ->get();
                $assigned_fees_types[$student->id]= $assigned_fees_type;

                $assigned_fees_group=DB::table('aramisc_fees_assigns')
                            ->where('student_id',$student->student_id)
                            ->where('record_id',$student->id)
                            ->join('aramisc_fees_masters','aramisc_fees_masters.id','=','aramisc_fees_assigns.fees_master_id')
                            ->join('aramisc_fees_groups','aramisc_fees_groups.id','=','aramisc_fees_masters.fees_group_id')
                            ->where('aramisc_fees_assigns.applied_discount','=',null)
                            ->distinct ('fees_group_id')
                            ->select('aramisc_fees_masters.id','aramisc_fees_groups.id as group_id','name','amount','aramisc_fees_assigns.student_id')
                            ->get();
            ...
            foreach ($datas as $data) {
                ...
                if (!gbv($data, 'checked') || !gv($data, 'fees_master_id')){
                    ...
                    if ($discount_info->type=='once') {
                        ...
                        $get_masters=DB::table('aramisc_fees_masters')->where('fees_group_id',$feesMasterId)->get();
    ...
    {
        $discount_unapplied_amount=DB::table('aramisc_fees_assign_discounts')->where('fees_discount_id',$request->fees_discount_id)->where('student_id',$request->student_id)->first();
