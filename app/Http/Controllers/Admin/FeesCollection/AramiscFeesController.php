...
class AramiscFeesController extends Controller
{
    ...
    public function feesPaymentStore(Request $request)
    {
        ...
        try {
            ...
            if(moduleStatusCheck('University')){
                $fees_assign = AramiscFeesAssign::where('fees_master_id',$request->master_id)
    ...
    public function feesPaymentDelete(Request $request)
    {
        try {
            ...
            if (checkAdmin()) {
                $payment = AramiscFeesPayment::find($request->id);
            }else{
    ...
    public function searchFeesDueCopy(Request $request)
    {
        try {
            ...
            $fees_dues = [];
            $dues_fees_master = AramiscFeesMaster::select('id', 'amount','date')
                ->where('academic_id', getAcademicId())
                ->where('school_id',Auth::user()->school_id)
                ->get();
            foreach ($students as $student) {
                foreach($dues_fees_master as $fees_master) {
                    ...
                    $fees_assign = AramiscFeesAssign::where('student_id', $student->student_id)
                    ...
                    $discount_amount = AramiscFeesAssign::where('student_id', $student->student_id)
                        ->where('record_id', $student->id)
                        ->where('academic_id', getAcademicId())
                        ->where('fees_master_id', @$fees_master->id)
                        ->where('school_id',Auth::user()->school_id)
                    ...
                    $amount = AramiscFeesPayment::where('active_status',1)
    ...
    public function searchFeesDue(Request $request)
    {
        if (moduleStatusCheck('University')) {
            ...
            try {
                ...
                $fees_assigns = AramiscFeesAssign::with('feesGroupMaster', 'recordDetail', 'feesGroupMaster.feesTypes')->get();
                foreach ($fees_assigns as $assignFees) {
    ...
    public function feesDueSearch(Request $request)
    {
        ...
        try {
            if(moduleStatusCheck('University')){
                ...
                $fees_master = AramiscFeesMaster::select('id', 'amount')
                ...
                foreach ($studentRecord as $record) {
                    $fees_master = AramiscFeesMaster::select('id', 'amount','date')
                        ->where('fees_group_id', $fees_group[0])
                        ->where('fees_type_id', $fees_group[1])
                        ->where('academic_id', getAcademicId())
                        ->where('school_id',Auth::user()->school_id)
                    ...
                    $fees_assign = AramiscFeesAssign::where('student_id', $record->student_id)
                    ...
                    $discount_amount = AramiscFeesAssign::where('student_id', $record->student_id)
                        ->where('record_id', $record->id)
                        ->where('academic_id', getAcademicId())
                        ->where('fees_master_id', $fees_master->id)
                        ->where('school_id',Auth::user()->school_id)
    ...
    public function sendDuesFeesEmail(Request $request){
        try{
            ...
            $students = AramiscStudent::with('parents')->whereIn('id', $request->student_list)->get();

            $fees_info = AramiscFeesMaster::with('feesType', 'feesGroups')->find($request->fees_master);
    ...
    public function feesPaymentInvoicePrint($id, $s_id)
    {
        ...
        try {
            ...
            $unapplied_discount_amount = AramiscFeesAssignDiscount::where('student_id',$student->student_id)
                ->where('record_id',$student->id)
                ->where('school_id',Auth::user()->school_id)
    ...
    public function updateSubPaymentModal(Request $request)
    {
        ...
        if($payment){
            ...
            $payment->save();
            $sm_fees_payment = AramiscFeesPayment::where('installment_payment_id',$payment->id)->first();
            if($sm_fees_payment){
