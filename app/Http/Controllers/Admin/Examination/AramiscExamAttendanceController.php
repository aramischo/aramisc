...
class AramiscExamAttendanceController extends Controller
{
    ...
    public function examAttendanceSearch(AramiscExamAttendanceSearchRequest $request)
    {
        try {
            if (moduleStatusCheck('University')) {
                ...
                $exam_schedules = universityFilter($AramiscExamSchedule, $request)
                ...
                $exam_schedules = AramiscExamSchedule::where('subject_id', $request->subject)
                ...
                $exam_attendance = AramiscExamAttendance::where('exam_id', $exam->id)
                    ->when($request->class, function ($q) use ($request) {
                        $q->where('class_id', $request->class);
                    })
                    ->when($request->section, function ($q) use ($request) {
    ...
    public function examAttendanceStore(Request $request)
    {
        try {
            if (moduleStatusCheck('University')) {
                ...
                $AramiscExamAttendance  = AramiscExamAttendance::query();
                $alreday_assigned = universityFilter($AramiscExamAttendance, $request)
                    ->where('un_subject_id', $request->un_subject_id)
                ...
                $exam_attendance->exam_id = $aramisc_exam->id;
                ...
                $exam = AramiscExam::where('exam_type_id', $request->exam_id)
                ...
                $alreday_assigned  = AramiscExamAttendance::where('exam_id', $exam->id)
                ...
                if ($request->section_id != '') {
                    if ($alreday_assigned == "") {
                        ...
                        $exam_attendance = AramiscExamAttendance::where('class_id', $request->class_id)
                            ->where('section_id', $request->section_id)
                            ->where('subject_id', $request->subject_id)
