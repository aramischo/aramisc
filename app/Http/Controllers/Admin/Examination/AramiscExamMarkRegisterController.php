...
class AramiscExamMarkRegisterController extends Controller
{
        ...
        {
            try{
                //$exams = AramiscExam::with('examType', 'class', 'section')->get();
    ...
    public function search(AddMarkRequest $request)
    {
        ...
        try {
            if (moduleStatusCheck('University')) {
                ...
                $exam_id = $aramisc_exam->id;
                $exam_type_id = $request->exam_type;
                $subject_id = $request->subject_id;
                $exam_type = AramiscExamType::find($request->exam_type);
                ...
                $exam_schedule = AramiscExamSchedule::where('exam_id', $aramisc_exam->id)
                                ->where('academic_id', getAcademicId())
                                ->first();
                ...
                if ($students->count() < 1 ) {
                    ...
                    $AramiscExamSetu = AramiscExamSetup::query();
            
                    $marks_entry_form = $AramiscExamSetu->where('un_semester_label_id',$request->un_semester_label_id);
                    ...
                    $marks_entry_form->where(
                        ...
                        $marks_entry_form = $AramiscExamSetu->get();
                ...
                if ($request->section=='') {
                    ...
                    $exam = $exam->first();
                    $exam_attendance = AramiscExamAttendance::where('class_id', $request->class)->where('section_id', $request->section)
                ...
                $exam_schedule = AramiscExamSchedule::where('exam_id', $request->exam)->where('class_id', $request->class)->where('section_id', $request->section)
                ...
                // return redirect()->back()->with('message-danger', 'Student is not found in according this class and section! Please add student in this section of that class.');
                    ...
                    if($request->section !=''){
                    $marks_entry_form = AramiscExamSetup::with('class','section')->where(
                        [
    ...
    public function store(Request $request)
    {
        ...
        try {
            if(moduleStatusCheck('University')){
                ...
                $subject_id = $request->subject_id;
                //$exam_id = AramiscExam::find($request->exam_id)->exam_type_id;
    ...
    try {
        if(moduleStatusCheck('University')){
            ...
            $examAttendance = AramiscExamAttendance::query();
            $exam_attendance= universityFilter($examAttendance, $request)
                            ->where('un_subject_id', $subject_id)
                            ->first();

            if ($exam_attendance) {
                $exam_attendance_child = AramiscExamAttendanceChild::where('exam_attendance_id', $exam_attendance->id)
                ->first();
            } else {
            ...
            $exam_schedule = universityFilter($AramiscExamSchedule, $request)
                            ->where('un_subject_id', $subject_id)
                            ->first();
            $request = $request;
            ...
            if ($students->count() == 0) {
                ...
                $marks_entry_form = universityFilter($AramiscExamSetup, $request)
