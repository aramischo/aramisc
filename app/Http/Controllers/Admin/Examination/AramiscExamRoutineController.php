...
use App\Http\Requests\Admin\Examination\AramiscExamScheduleSearchRequest;
...
class AramiscExamRoutineController extends Controller
{
    ...
    public function examScheduleReportSearch(AramiscExamScheduleSearchRequest $request)
    {
        try {
    ...
    public function examScheduleSearch(AramiscExamScheduleSearchRequest $request)
    {
        try {
            if (moduleStatusCheck('University')) {
                $data = [];
                ...
                $subjects = universityFilter($AramiscExam, $request)
        ...
        try {
            if (moduleStatusCheck('University')) {
                ...
                foreach ($request->routine as $routine_data) {
                    ...
                    $aramisc_exam = AramiscExam::where('exam_type_id', $exam_type_id)
                    ...
                    $exam_routine->exam_id = $aramisc_exam->id;
    ...
    function universityExamRoutinePrint($un_semester_label_id, $un_section_id, $exam_term_id)
    {
        try {
            ...
            $section_name   = $un_section_id != 0 ? AramiscSection::find($un_section_id)->section_name : 'All Sections';
            $exam_schedules = AramiscExamSchedule::where('school_id', Auth::user()->school_id)
