...
class AramiscRolePermissionController extends Controller
{
    ...
    public function assignPermission(Request $request,$id){
		...
		try{
			...
			$modulesRole = AramiscModule::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
			$role_permissions = AramiscRolePermission::where('role_id', $id)->where('school_id',Auth::user()->school_id)->get();
			$already_assigned = [];
		...
		try{
			...
			// AramiscRolePermission::where('role_id', $request->role_id)->delete();
			 if (checkAdmin()) {
				...
				AramiscRolePermission::where('role_id', $request->role_id)->where('school_id',Auth::user()->school_id)->delete();
			}
