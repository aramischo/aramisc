...
use App\Http\Requests\Admin\Inventory\AramiscItemSellRequest;
use App\Http\Requests\Admin\Inventory\AramiscItemIssueRequest;

class AramiscItemSellController extends Controller
{
    ...
    public function saveItemSellData(AramiscItemSellRequest $request)
    {
        try {
            $total_paid = '';
            ...
            if ($results) {
                ...
                for ($i = 0; $i < $item_ids; $i++) {
                    if (!empty($request->item_id[$i])) {
                        $itemSellChild = new AramiscItemSellChild();
    ...
    public function checkProductQuantity()
    {
        try {
            ...
            $product_quantity = AramiscItem::select('total_in_stock')->where('id', $product_id)->first();
    ...
    {
        try {
            $allItemSellLists = AramiscItemSell::where('active_status', '=', 1)
                                ->where('school_id', Auth::user()->school_id)
                                ->orderby('id','DESC')
                                ->get();
    ...
    public function viewItemSell($id)
    {
        try {
            ...
            $viewData = AramiscItemSell::find($id);            
            $editDataChildren = AramiscItemSellChild::with('items')->where('item_sell_id', $id)->get();
    ...
    public function editItemSell(Request $request, $id)
    {
        try {
            ...
            if ($editData->role_id == 2) {
                $studentClassSection = AramiscStudent::where('id', $editData->student_staff_id)->first();
            ...
            $editDataChildren = AramiscItemSellChild::where('item_sell_id', $id)
    ...
    public function UpdateItemSellData(AramiscItemSellRequest $request)
    {
       // DB::statement('SET FOREIGN_KEY_CHECKS=0;');
        ...
        try {
            ...
            $itemSells = AramiscItemSell::find($request->id);
            $itemSells->role_id = $request->role_id;
            $itemSells->student_staff_id = $student_staff_id;
            $itemSells->reference_no = $request->reference_no;
            $itemSells->sell_date = date('Y-m-d', strtotime($request->sell_date));
            ...
            if ($results) {
                AramiscItemSellChild::where('item_sell_id', $itemSells->id)->delete();
                $item_ids = count($request->item_id);
                for ($i = 0; $i < $item_ids; $i++) {
                    if (!empty($request->item_id[$i])) {
                        $itemSellChild = new AramiscItemSellChild();
    ...
    {
        try {
            $viewData = AramiscItemReceive::where('id', $id)->where('school_id', Auth::user()->school_id)->first();
    ...
    public function itemSellPayment($id)
    {
        try {
            ...
            $paymentDue = AramiscItemSell::select('total_due')
                        ->where('id', $id)
                        ->where('school_id', Auth::user()->school_id)
                        ->first();
    ...
    public function saveItemSellPayment(Request $request)
    {
       // DB::statement('SET FOREIGN_KEY_CHECKS=0;');
        try {
            ...
            if (checkAdmin()) {
                $itemPaymentDue = AramiscItemSell::find($request->item_sell_id);
            } else {
                $itemPaymentDue = AramiscItemSell::where('id', $request->item_sell_id)->where('school_id', Auth::user()->school_id)->first();
            }
            ...
            // check if full paid
            $itemReceives = AramiscItemSell::find($request->item_sell_id);
    ...
    {
        $allitems = AramiscItem::where('item_category_id', '=', $request->id)->where('school_id', Auth::user()->school_id)->get();
        $items = [];
        foreach ($allitems as $item) {
            $items[] = AramiscItem::find($item->id);
        }
    ...
    public function saveItemIssueData(AramiscItemIssueRequest $request)
    {
    ...
    public function deleteSellPayment()
    {
        ...
        try {
            ...
            $itemSellData = AramiscItemSell::find($item_receive_sell_id);
    ...
    public function cancelItemSell($id)
    {
        try {
            ...
            if ($results) {
                $itemSellChild = AramiscItemSellChild::where('item_sell_id', $id)->where('school_id', Auth::user()->school_id)->get();
    ...
    # This is for upadate database aramisc_item_sells && aramisc_item_sell_children table for the issue of  float/double datatype only stores 8 digits.

    public static function updateAramiscItemSellDatabase()
    {
        try {
            Schema::table('aramisc_item_sells', function (Blueprint $table) {
    ...
    public static function updateAramiscItemSellChildrenDatabase()
    {
        try {
            Schema::table('aramisc_item_sell_children', function (Blueprint $table) {
