...
class AramiscQuestionLevelController extends Controller
{
    ...
    {
        try{
            $levels = AramiscQuestionLevel::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
    ...
    {
        $request->validate([
            'level' => "required|unique:aramisc_question_levels"
        ]);
        try{
            $level = new AramiscQuestionLevel();
    ...
    {
        try{
            $level = AramiscQuestionLevel::find($id);
            $levels = AramiscQuestionLevel::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
    ...
    public function update(Request $request, $id)
    {
        try{
            $request->validate([
                'level' => "required|unique:aramisc_question_levels,level,".$request->id
            ]);

            $level = AramiscQuestionLevel::find($request->id);
            $level->level = $request->level;
            $result = $level->save();
            if($result){
                Toastr::success('Operation successful', 'Success');
    ...
    public function destroy($id)
    {
        ...
        try{
            $level = AramiscQuestionLevel::destroy($id);
            if($level){
                Toastr::success('Operation successful', 'Success');
