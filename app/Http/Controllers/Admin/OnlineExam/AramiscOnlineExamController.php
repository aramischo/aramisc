...
class AramiscOnlineExamController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            if (teacherAccess()) {
                ...
                $online_exams = AramiscOnlineExam::where('status', '!=', 2)
                ...
                $online_exams = AramiscOnlineExam::with('class', 'section', 'subject')->where('status', '!=', 2)->get();
            }
    ...
    {
        try {
            $online_exam = AramiscOnlineExam::find($id);
            $assigned_questions = AramiscOnlineExamQuestionAssign::where('online_exam_id', $online_exam->id)
    ...
    function universityOnlineExamStore($request)
    {
        ...
        try {
            foreach ($request->un_section_ids as $section) {
                $online_exam = new AramiscOnlineExam();
    ...
    {
        $time_zone_setup = AramiscGeneralSettings::with('timeZone')->where('school_id', Auth::user()->school_id)
            ->first();
        date_default_timezone_set(@$time_zone_setup->timeZone->time_zone);
        try {
            $online_exams = AramiscOnlineExam::where('school_id', Auth::user()->school_id)->get();
            $online_exam = AramiscOnlineExam::find($id);
            if (teacherAccess()) {
                $teacher_info = AramiscStaff::where('user_id', Auth::user()->id)->first();
                $classes = $teacher_info->classes;
            ...
            $sections = AramiscAssignSubject::where('class_id', $online_exam->class_id)->where('subject_id', $online_exam->subject_id)->get();
            $subjects = AramiscAssignSubject::where('class_id', $online_exam->class_id)->where('section_id', $online_exam->section_id)->select('subject_id')->distinct('subject_id')->get();
            $present_date_time = date("Y-m-d H:i:s");
            $present_time = date("H:i:s");
    ...
    public function update(AramiscOnlineExamRequest $request, $id)
    {
        if (moduleStatusCheck('University')) {
    ...
    public function delete(Request $request)
    {
        try {
            ...
            try {
                if ($tables == null) {
                    $delete_query = AramiscOnlineExam::findOrFail($request->online_exam_id);
                    if ($delete_query) {
                        $delete_query->delete();
                    }
                    Toastr::success('Operation successful', 'Success');
    ...
    public function manageOnlineExamQuestion($id)
    {
        try {
            $online_exam = AramiscOnlineExam::find($id);
            $question_banks = AramiscQuestionBank::with('questionGroup')->where('class_id', $online_exam->class_id)
                ->where('section_id', $online_exam->section_id)
                ->get();
            $already_assigned = AramiscOnlineExamQuestionAssign::where('online_exam_id', $id)
    ...
    public function manageOnlineExamQuestionStore(Request $request)
    {
        try {
            if ($request->question_type != 'M') {
                $online_question = new AramiscOnlineExamQuestion();
                ...
                try {
                    ...
                    if (isset($request->option)) {
                        foreach ($request->option as $option) {
                            ...
                            $online_question_option = new AramiscOnlineExamQuestionMuOption();
    ...
    public function onlineExamPublish($id)
    {
        try {
            ...
            date_default_timezone_set($time_zone_setup->time_zone);
            $present_date_time = date("Y-m-d H:i:s");
            $publish = AramiscOnlineExam::find($id);
    ...
    {
        try {
            $online_exam_question = AramiscOnlineExamQuestion::find($id);
    ...
    public function onlineExamQuestionEdit(Request $request)
    {
        try {
            if ($request->question_type != 'M') {
                if (checkAdmin()) {
                    $online_question = AramiscOnlineExam::find($request->id);
                } else {
                    $online_question = AramiscOnlineExam::where('id', $request->id)->where('school_id', Auth::user()->school_id)->first();
                }
                $online_question->online_exam_id = $request->online_exam_id;
                $online_question->type = $request->question_type;
                $online_question->mark = $request->mark;
                ...
                try {
                    if (checkAdmin()) {
                        $online_question = AramiscOnlineExamQuestion::find($request->id);
                    } else {
                        $online_question = AramiscOnlineExamQuestion::where('id', $request->id)->where('school_id', Auth::user()->school_id)->first();
                    }
                    $online_question->online_exam_id = $request->online_exam_id;
                    $online_question->type = $request->question_type;
                    $online_question->mark = $request->mark;
                    ...
                    $online_question->save();
                    $online_question->toArray();
                    AramiscOnlineExamQuestionMuOption::where('online_exam_question_id', $online_question->id)->delete();
                    $i = 0;
                    if (isset($request->option)) {
                        foreach ($request->option as $option) {
                            $i++;
    ...
    public function onlineExamQuestionDelete(Request $request)
    {
        try {
            if (checkAdmin()) {
                $online_exam_question = AramiscOnlineExamQuestion::find($request->id);
            } else {
                $online_exam_question = AramiscOnlineExamQuestion::where('id', $request->id)->where('school_id', Auth::user()->school_id)->first();
            }
            if ($online_exam_question->type == "M") {
                AramiscOnlineExamQuestionMuOption::where('online_exam_question_id', $online_exam_question->id)->delete();
    ...
    {
        try {
            $online_exam_question = AramiscOnlineExam::find($id);
            ...
            $present_students = [];
            $take_exam = AramiscStudentTakeOnlineExam::whereIn('student_id', $student_ids)
    ...
    public function onlineExamMarksStore(Request $request)
    {
        try {
            if (checkAdmin()) {
                AramiscOnlineExamMark::where('exam_id', $request->exam_id)->delete();
            } else {
                AramiscOnlineExamMark::where('exam_id', $request->exam_id)->where('school_id', Auth::user()->school_id)->delete();
            }
            $counter = 0;
            foreach ($request->students as $student) {
                ...
                $abs = 'abs_' . $counter;
                $online_mark = new AramiscOnlineExamMark();
    ...
    public function onlineExamResult($id)
    {
        try {
            ...
            $student_ids = $studentRecords->pluck('student_id')->toArray();
            $present_students = [];
            $students = AramiscStudent::whereIn('id', $studentRecords->pluck('student_id')->toArray())->get();
            foreach ($students as $student) {
    ...
    public function onlineExamQuestionAssign(Request $request)
    {
        try {
            if ($request->checkbox) {
                $assign = new AramiscOnlineExamQuestionAssign();
                ...
                AramiscOnlineExamQuestionAssign::where('question_bank_id', $request->questions)->delete();
                if (checkAdmin()) {
                    AramiscOnlineExamQuestionAssign::where('question_bank_id', $request->questions)->delete();
                } else {
                    AramiscOnlineExamQuestionAssign::where('question_bank_id', $request->questions)->where('school_id', Auth::user()->school_id)->delete();
                }
            }
    ...
    {
        try {
            $question_bank = AramiscQuestionBank::find($id);
    ...
    {
        $online_exam_info = AramiscOnlineExam::find($exam_id);
        try {
            $online_take_exam_mark = AramiscStudentTakeOnlineExam::where('online_exam_id', $exam_id)
                ...
                ->first();
            $assign_questions = AramiscOnlineExamQuestionAssign::where('online_exam_id', $exam_id)->get();
            $total_mark = 0;
            foreach ($assign_questions as $q) {
                $marks = AramiscQuestionBank::find($q->question_bank_id)->marks;
                $total_mark += $marks;
            }
            if ($online_exam_info->auto_mark == 1) {
                $take_online_exam = AramiscStudentTakeOnlineExam::where('online_exam_id', $exam_id)->where('student_id', $s_id)
                ->when(moduleStatusCheck('University'), function ($query) {
                    return $query->where('un_academic_id', getAcademicId());
                })
                ->when(!moduleStatusCheck('University'), function ($query) {
    ...
    {
        try {
            $exam_questions = AramiscOnlineExamQuestionAssign::where('online_exam_id', $request->online_exam_id)->get();
            foreach ($exam_questions as $question) {
                ...
                $exam_info = AramiscOnlineExam::find($request->online_exam_id);
                $question_info = AramiscQuestionBank::find($question_id);
            ...
            $online_take_exam_mark = AramiscStudentTakeOnlineExam::where('online_exam_id', $request->online_exam_id)->where('student_id', $request->student_id)->where('academic_id', getAcademicId())->first();
            ...
            $studentInfo = AramiscStudent::find($request->student_id);
    ...
    function universityOnlineExamReportSearch($request){
        ...
        try {
            ...
            $present_date_time = date("Y-m-d H:i:s");
            $student_ids = AramiscStudentReportController::SemesterLabelSectionStudent($request);
            ...
            if (moduleStatusCheck('OnlineExam') == TRUE) {
                ...
                $online_exam = AramiscOnlineExam::where('un_semester_label_id', $request->un_semester_label_id)->where('un_section_id', $request->un_section_id)->where('id', $request->exam)
                ...
                $exams = AramiscOnlineExam::where('active_status', 1)->where('un_academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            }
    ...
    public function onlineExamReportSearch(Request $request)
    {
        if (moduleStatusCheck('University')) {
            ...
            try {
                ...
                $students = AramiscStudent::whereIn('id', $student_ids)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
                ...
                $sec = AramiscSection::find($request->section);
    ...
    public function onlineExamDatatable()
    {
        try {
            if (teacherAccess()) {
                $teacher_info = AramiscStaff::where('user_id', Auth::user()->id)->first();
                $online_exams = AramiscOnlineExam::query();
