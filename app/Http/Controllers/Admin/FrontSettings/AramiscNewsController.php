...
use App\Http\Requests\Admin\FrontSettings\AramiscNewsRequest;
use DataTables;

class AramiscNewsController extends Controller
{
    ...
    public function index()
    {
        ...
        try{
            ...
            $news_category = AramiscNewsCategory::where('school_id', app('school')->id)->get();
    ...
    public function store(AramiscNewsRequest $request)
    {
        try{
    ...
    public function update(Request $request)
    {
        try{
            ...
            $news = AramiscNews::find($request->id);
            $news->news_title = $request->title;
            $news->category_id = $request->category_id;
            $news->publish_date = $newformat;           
    ...
    public function commentListDatatable()
    {
        if(request()->comment_filter_type){
            if(request()->comment_filter_type == 'approve'){
                $news_comments = AramiscNewsComment::with('news', 'user')
    ...
    {
        try{
            $commentData = AramiscNewsComment::find($request->comment_id);
    ...
    {
        try{
            $parentDatas = AramiscNewsComment::where('parent_id', $request->comment_id)->get();
            if($parentDatas->count() > 0){
                foreach($parentDatas as $parentData){
                    $parentData->delete();
                }
            }
            AramiscNewsComment::destroy($request->comment_id);
    ...
    {
        try{
            $data = AramiscNewsComment::find($id);
            if($data->status == 1){
                $data->status = 0;
                $data->update();
            }else{
    ...
    {
        try {
            $category_id = AramiscNewsCategory:: find($id);
            $newsCtaegories = AramiscNews::where('category_id',$category_id->id)
