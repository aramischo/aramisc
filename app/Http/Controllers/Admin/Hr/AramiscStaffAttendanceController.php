...
class AramiscStaffAttendanceController extends Controller
{
    ...
    public function staffAttendanceSearch(staffAttendanceSearchRequest $request)
    {
        try {
            ...
            $staffs = AramiscStaff::with('DateWiseStaffAttendance', 'roles')
    ...
    public function staffAttendanceStore(Request $request)
    {
        ...
        try {
            foreach ($request->id as $staff) {
                ...
                $staffInfo = AramiscStaff::find($staff);
                $compact['slug'] = 'staff';
    ...
    {
        $staffs = AramiscStaff::where('role_id', $request->role_id)
    ...
    public function staffAttendanceReportSearch(staffAttendanceReportSearchRequest $request)
    {
        try {
            ...
            $staffs = AramiscStaff::where('role_id', $request->role)->where('school_id', Auth::user()->school_id)->get(['id', 'staff_no']);

            $attendances = [];
            foreach ($staffs as $staff) {
                $attendance = AramiscStaffAttendence::with('staffInfo')->where('staff_id', $staff->id)->where('attendence_date', 'like', $request->year . '-' . $request->month . '%')->where('school_id', Auth::user()->school_id)->get();
    ...
    public function staffAttendancePrint($role_id, $month, $year)
    {
        ...
        try {
            ...
            $staffs = AramiscStaff::where('role_id', $role_id)->where('school_id', Auth::user()->school_id)->get();
            $role = AramiscRole::find($role_id);

            $attendances = [];
            foreach ($staffs as $staff) {
                $attendance = AramiscStaffAttendence::where('staff_id', $staff->id)->where('attendence_date', 'like', $year . '-' . $month . '%')->where('school_id', Auth::user()->school_id)->get();
                if (count($attendance) != 0) {
                    $attendances[] = $attendance;
                }
            }
    ...
    public function teacherMyAttendanceSearchAPI(Request $request, $id = null)
    {
        ...
        try {
            $teacher = AramiscStaff::where('user_id', $id)->where('school_id', Auth::user()->school_id)->first();
    ...
    public function staffAttendanceBulkStore(staffAttendanceBulkStoreRequest $request)
    {
        ...
        if ($file_type != 'csv' && $file_type != 'xlsx' && $file_type != 'xls') {
            ...
            try {
                ...
                $data = AramiscStaffAttendanceImport::get();
                if (!empty($data)) {
                    DB::beginTransaction();
                    $staffs = AramiscStaff::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
                    $all_staff_ids = [];
                    $present_staffs = [];
                        ...
                        foreach ($data as $key => $value) {
                            if (!empty($value)) {
                                if (date('d/m/Y', strtotime($request->attendance_date)) == date('d/m/Y', strtotime($value->attendence_date))) {
                                    $staff = AramiscStaff::find($value->staff_id);
                                    $attendance = AramiscStaffAttendence::where('staff_id', $staff->id)
