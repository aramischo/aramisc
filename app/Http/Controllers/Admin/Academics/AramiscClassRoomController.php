...
use App\Http\Requests\Admin\Academics\AramiscClassRoomRequest;

class AramiscClassRoomController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $class_rooms = AramiscClassRoom::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
    ...
    public function store(AramiscClassRoomRequest $request)
    {
        try {
    ...
    {
        try {
            $class_room = AramiscClassRoom::find($id);
            $class_rooms = AramiscClassRoom::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
    ...
    public function update(AramiscClassRoomRequest $request, $id)
    {
        try {
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            try {
                if ($tables==null) {
                      $delete_query = AramiscClassRoom::destroy($id);
                    if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                        if ($delete_query) {
