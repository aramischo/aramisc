...
class AramiscAssignSubjectController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        try {
            $sections = AramiscClassSection::where('class_id', $request->class)->with('sectionName', 'className')
                ->when($request->section, function ($q) use ($request) {
                    $q->where('section_id', $request->section);
                })->get();
            $assign_subjects = AramiscAssignSubject::where('class_id', $request->class)
                ->when($request->section, function ($q) use ($request) {
                    $q->where('section_id', $request->section);
                })->get();

            $subjects = AramiscSubject::where('active_status', 1)->where('school_id', Auth::user()->school_id)->where('academic_id', getAcademicId())->get();
            $teachers = AramiscStaff::where('active_status', 1)
                ->where(function ($q) {
