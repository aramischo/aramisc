...
use App\Http\Requests\Admin\Academics\AramiscAssignClassTeacherRequest;
use App\Traits\NotificationSend;

class AramiscAssignClassTeacherController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $teachers = AramiscStaff::status()->where(function ($q) {
                $q->where('role_id', 4)->orWhere('previous_role_id', 4);
            })->get();
            $assign_class_teachers = AramiscAssignClassTeacher::with('class', 'section', 'classTeachers')->where('academic_id', getAcademicId())->status()->orderBy('class_id', 'ASC')->orderBy('section_id', 'ASC')->get();
    ...
    public function store(AramiscAssignClassTeacherRequest $request)
    {
        DB::beginTransaction();
        try {
            $assigned_class_teacher = AramiscAssignClassTeacher::where('active_status', 1)
    ...
    public function edit(Request $request, $id)
    {
        ...
        try {
            ...
            $assign_class_teachers = AramiscAssignClassTeacher::with('class', 'section', 'classTeachers')->where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
    ...
    public function update(AramiscAssignClassTeacherRequest $request, $id)
    {

        $is_duplicate = AramiscAssignClassTeacher::where('school_id', Auth::user()->school_id)->where('academic_id', getAcademicId())->where('class_id', $request->class)->where('section_id', $request->section)->where('id', '!=', $request->id)->first();
        if ($is_duplicate) {
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            try {
                ...
                $delete_query = AramiscClassTeacher::where('assign_class_teacher_id', $id)->delete();
