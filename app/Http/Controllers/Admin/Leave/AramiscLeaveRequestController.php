...
use App\Http\Requests\Admin\Leave\AramiscLeaveRequest as FormRequest;

class AramiscLeaveRequestController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $user = Auth::user();
            if ($user) {
                $my_leaves = AramiscLeaveDefine::with('leaveType')->where('user_id', $user->id)->where('role_id', $user->role_id)->where('school_id', Auth::user()->school_id)->get();
                $apply_leaves = AramiscLeaveRequest::with('leaveDefine')->where('role_id', $user->role_id)->where('active_status', 1)
    ...
    {
        try {
            $leaveDefine = AramiscLeaveDefine::where('id', $request->leave_define_id)->first();
    ...
    public function show(Request $request, $id)
    {
        ...
        try {
            $user = Auth::user();
            if ($user) {
                $my_leaves = AramiscLeaveDefine::where('user_id', $user->id)->where('role_id', $user->role_id)->where('school_id', Auth::user()->school_id)->get();
                $apply_leaves = AramiscLeaveRequest::with('leaveDefine')->where('role_id', $user->role_id)->where('active_status', 1)
                ->where('school_id', Auth::user()->school_id)->has('leaveDefine')->where('staff_id', Auth::user()->id)->get();
                $leave_types = AramiscLeaveDefine::where('role_id', $user->role_id)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            }
