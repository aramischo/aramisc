...
use App\Http\Requests\Admin\Leave\AramiscLeaveDefineRequest;
use Modules\University\Repositories\Interfaces\UnCommonRepositoryInterface;

class AramiscLeaveDefineController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $roles = AramiscRole::where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 3)->where('id', '!=', GlobalVariable::isAlumni())->where(function ($q) {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
            $classes = AramiscClass::get(['id', 'class_name']);
    ...
    public function store(AramiscLeaveDefineRequest $request)
    {
        try {
            $previous =  AramiscLeaveDefine::where('role_id', $request->member_type)
            ...
            if (is_numeric($request->student)  || is_numeric($request->staff)) {
                if (is_null($previous)) {
                    $leave_define = new AramiscLeaveDefine();
    ...
    public function show(Request $request, $id)
    {
        try {
            ...
            $leave_defines = AramiscLeaveDefine::get();
            $leave_define = AramiscLeaveDefine::find($id);

            $user = User::find($leave_define->user_id);
            $student = null;
            $staff = null;
            if ($leave_define->role_id == 2) {
                $aramiscStudent = AramiscStudent::where('user_id', $user->id)->first();
                $student = StudentRecord::where('student_id', $aramiscStudent->id)
    ...
    public function update(AramiscLeaveDefineRequest $request, $id)
    {
        try {
            if (checkAdmin()) {
                $leave_define = AramiscLeaveDefine::find($request->id);
            } else {
                $leave_define = AramiscLeaveDefine::where('id', $request->id)->where('school_id', Auth::user()->school_id)->first();
            }
            $leave_define->type_id = $request->leave_type;
            $leave_define->days = $request->days;
            if (moduleStatusCheck('University')) {
    ...
    public function destroy(Request $request)
    {
        ...
        try {
            ...
            try {
                if ($tables == null) {
                    if (checkAdmin()) {
                        AramiscLeaveDefine::destroy($id);
                    } else {
                        AramiscLeaveDefine::where('id', $id)->where('school_id', Auth::user()->school_id)->delete();
                    }
