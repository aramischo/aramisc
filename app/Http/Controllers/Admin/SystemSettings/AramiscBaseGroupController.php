...
class AramiscBaseGroupController extends Controller
{
        ...
        try{
            // $base_group = AramiscBaseGroup::find($id);
             if (checkAdmin()) {
                ...
                $base_group = AramiscBaseGroup::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
            }
        ...
        try{
            // $base_group = AramiscBaseGroup::find($request->id);
             if (checkAdmin()) {
                ...
                $base_group = AramiscBaseGroup::where('id',$request->id)->where('school_id',Auth::user()->school_id)->first();
            }
            $base_group->name = $request->name;
            $result = $base_group->save();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
    ...
    public function delete(Request $request,$id){
        ...
        try {
            if(ApiBaseMethod::checkUrl($request->fullUrl())) {
                // $result = $delete_query = AramiscBaseGroup::destroy($id);
                 if (checkAdmin()) {
                    $result = AramiscBaseGroup::destroy($id);
                }else{
                    $result = AramiscBaseGroup::where('id',$id)->where('school_id',Auth::user()->school_id)->delete();
                }
                if($result){
