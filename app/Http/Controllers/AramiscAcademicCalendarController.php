...
class AramiscAcademicCalendarController extends Controller
{
    ...
    public function storeAcademicCalendarSettings(Request $request){
        try{
            foreach(gv($request, 'setting') as $key => $data){
                $settings = AramiscCalendarSetting::where('menu_name', $key)->first();
	...
	public function getCalendarSettings($menu_name){
        return AramiscCalendarSetting::where('menu_name', $menu_name)
    ...
    public function calenderData(){
        ...
        if($studyMaterialSettings->status == 1){
            $studyMaterials = AramiscTeacherUploadContent::with('classes', 'sections')
            ->when($userRoleId == 2, function($s) use ($studentRecords){
                $class_ids = $studentRecords->pluck('class_id')->toArray();
                $section_ids = $studentRecords->pluck('section_id')->toArray();
        ...
        if($eventSettings->status == 1){
            $allEvents = AramiscEvent::when($roleInfo->name != 'Super admin', function($a) use($roleInfo){
        ...
        if($holidaySettings->status == 1){
            $holidays = AramiscHoliday::where('active_status', 1)->get(['holiday_title', 'details', 'from_date', 'to_date', 'upload_image_file']);
        }

        if($examSettings->status == 1){
            $examRounines = AramiscExamSchedule::with('class', 'section', 'subject', 'examType', 'teacher', 'classRoom')
            ->when($userRoleId == 2, function($s) use ($studentRecords){
                $class_ids = $studentRecords->pluck('class_id')->toArray();
                $section_ids = $studentRecords->pluck('section_id')->toArray();
                $s->whereIn('class_id', $class_ids);
        ...
        if($librarySettings->status == 1){
            $libraryDatas = AramiscBookIssue::with('user')
