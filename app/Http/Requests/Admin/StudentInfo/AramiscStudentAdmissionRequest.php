...
class AramiscStudentAdmissionRequest extends FormRequest
{
    /**
    ...
    public function rules(Request $request)
    {
        ...
        if ($this->id) {
            $student = AramiscStudent::with('parents', 'studentRecords')->findOrFail($this->id);
        ...
        $field = AramiscStudentRegistrationField::where('school_id', $school_id)
        ...
        $rules= [
            ...
            'admission_number' => ['integer', Rule::unique('aramisc_students', 'admission_no')->ignore(optional($student)->id)->where('school_id', $school_id), Rule::requiredIf(function () use ($field) {
        ...
        if ($user_role_id !=2 || $user_role_id !=10) {
            ...
            }), Rule::unique('aramisc_students', 'roll_no')->ignore(optional($student)->id)->where('school_id', $school_id)->where('academic_id', $academic_id)->whereIn('class_id', $class_ids)->whereIn('section_id', $section_ids)],
            ];
        }
