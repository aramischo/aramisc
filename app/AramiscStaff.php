...
class AramiscStaff extends Model
{
    use HasFactory;
    protected $casts = [
        'id'            => 'integer',
    ...
    protected $table = "aramisc_staffs";
    ...
    {
        return $this->belongsTo('App\AramiscHumanDepartment', 'department_id', 'id')->withDefault();
    }

    public function designations()
    {
        return $this->belongsTo('App\AramiscDesignation', 'designation_id', 'id')->withDefault();
    }
    ...
    {
        return $this->hasMany(AramiscStaffAttendence::class, 'staff_id')->where('school_id', auth()->user()->school_id);
    }
    ...
    {
        return $this->hasMany('App\AramiscAssignSubject', 'teacher_id', 'id')
    ...
    {
        return $this->hasOne('App\AramiscStaffAttendence', 'staff_id', 'id')->where('attendence_date', date('Y-m-d', strtotime(request()->attendance_date)));
    }

    public function DateWiseStaffAttendanceReport()
    {
        return $this->hasOne('App\AramiscStaffAttendence', 'staff_id', 'id')->where('attendence_date', 'like', request()->year . '-' . request()->month . '%');
    }

    public function payrollStatus()
    {

        return $this->hasOne('App\AramiscHrPayrollGenerate', 'staff_id', 'id')
