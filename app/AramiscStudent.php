...
class AramiscStudent extends Model
{
    ...
    protected $table = 'aramisc_students';
    protected $fillable = [];
    ...
    {
        return $this->belongsTo('App\AramiscStudentCategory', 'student_category_id', 'id')->withDefault();
    }

    public function group()
    {
        return $this->belongsTo('App\AramiscStudentGroup', 'student_group_id', 'id');
    }
    ...
    {
        return $this->belongsTo('App\AramiscRoute', 'route_list_id', 'id');
    }
    ...
    {
        return $this->belongsTo('App\AramiscDormitoryList', 'dormitory_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\AramiscRoomList', 'dormitory_id', 'dormitory_id');
    }

    public function room()
    {
        return $this->belongsTo('App\AramiscRoomList', 'room_id', 'id');
    }

    public function attendances()
    {
        return $this->hasMany(AramiscStudentAttendance::class, 'student_id');
    }

    public function forwardBalance()
    {
        return $this->belongsTo('App\AramiscFeesCarryForward', 'id', 'student_id');
    }
    ...
    {
        return $this->hasMany('App\AramiscFeesAssign', 'student_id', 'id')->where('academic_id',getAcademicId());
    }

    public function feesAssignDiscount()
    {
        return $this->hasMany('App\AramiscFeesAssignDiscount', 'student_id', 'id')->where('academic_id',getAcademicId());
    }

    public function studentDocument()
    {
        return $this->hasMany('App\AramiscStudentDocument', 'student_staff_id', 'id')->where('academic_id', getAcademicId());
    }

    public function studentTimeline()
    {
        return $this->hasMany('App\AramiscStudentTimeline', 'staff_student_id', 'id');
    }
    ...
    {
        return $this->hasMany(AramiscExamSchedule::class, 'class_id', 'class_id')
            ->where('section_id', $this->section_id);
    }
    ...
    {
        return $this->hasMany(AramiscBookIssue::class, 'member_id', 'user_id')->where('issue_status', 'I');
    }

    public function examSchedule()
    {
        return $this->hasMany(AramiscExamSchedule::class, 'class_id', 'class_id')->where('section_id', $this->section_id);
    }
    ...
    {
        return $this->hasMany(AramiscStudentAttendance::class, 'student_id')->where('attendance_date', 'like', date('Y') . '-' . date('m') . '%')
    ...
    public static function totalFees($feesAssigns)
    {
        try {
            ...
            foreach ($feesAssigns as $feesAssign) {
                $master = AramiscFeesMaster::select('fees_group_id', 'amount', 'date')->where('id', $feesAssign->fees_master_id)->first();
    ...
    public function getTotalDiscount($id)
    {
        $amount = 0;
        foreach ($this->feesAssign as $feesAssign) {
            $amount += AramiscFeesAssign::where('fees_type_id', $feesAssign->feesGroupMaster->fees_type_id)->where('student_id', $id)->sum('discount_amount');
        }
        return $amount;
    }
    ...
    public function getTotalFine($id)
    {
        $amount = 0;
        foreach ($this->feesAssign as $feesAssign) {
            $amount += AramiscFeesPayment::where('active_status', 1)->where('fees_type_id', $feesAssign->feesGroupMaster->fees_type_id)->where('student_id', $id)->sum('fine');
        }
        return $amount;
    }
    ...
    public function getTotalDeposit($id)
    {
        $amount = 0;
        foreach ($this->feesAssign as $feesAssign) {
            $amount += AramiscFeesPayment::where('active_status', 1)->where('fees_type_id', $feesAssign->feesGroupMaster->fees_type_id)->where('student_id', $id)->sum('amount');
        }
        return $amount;
    }
    ...
    public static function totalDeposit($feesAssigns, $student_id)
    {
        ...
        try {
            $amount = 0;
            foreach ($feesAssigns as $feesAssign) {
                $fees_type = AramiscFeesMaster::select('fees_type_id')->where('id', $feesAssign->fees_master_id)->first();
                $amount += AramiscFeesPayment::where('active_status', 1)->where('fees_type_id', $fees_type->fees_type_id)->where('student_id', $student_id)->sum('amount');
            }
            return $amount;
        } catch (\Exception $e) {
            $data = [];
    ...
    public static function totalDiscount($feesAssigns, $student_id)
    {
        ...
        try {
            $amount = 0;
            foreach ($feesAssigns as $feesAssign) {
                $amount = AramiscFeesAssign::where('student_id', $student_id)->sum('applied_discount');
            }
            return $amount;
        } catch (\Exception $e) {
            $data = [];
    ...
    public static function totalFine($feesAssigns, $student_id)
    {
        ...
        try {
            ...
            foreach ($feesAssigns as $feesAssign) {
                $fees_type = AramiscFeesMaster::select('fees_type_id')->where('id', $feesAssign->fees_master_id)->first();
                $amount += AramiscFeesPayment::where('active_status', 1)->where('fees_type_id', $fees_type->fees_type_id)->where('student_id', $student_id)->sum('fine');
            }
            return $amount;
        } catch (\Exception $e) {
            $data = [];
    ...
    {
        try {
            return AramiscExamScheduleSubject::where('exam_schedule_id', $exam_id)->where('subject_id', $sb_id)->first();
        } catch (\Exception $e) {
            $data = [];
            return $data;
        }
    ...
    {
        try {
            return AramiscExamSetup::where('exam_term_id', $exam_id)->where('subject_id', $sb_id)->first();
        } catch (\Exception $e) {
            $data = [];
            return $data;
        }
    ...
    public static function un_fullMarksBySubject($exam_id, $sb_id, $request)
    {
        try {
            ...
            return universityFilter($AramiscExamSetup, $request)
    ...
    {
        try {
            $AramiscExamSchedule = AramiscExamSchedule::query();
            $schedule = universityFilter($AramiscExamSchedule, $request)
                ->where('exam_term_id', $exam_id)
                ->where('un_subject_id', $sb_id)
                ->first();
            return $schedule;
    ...
    {
        try {
            $schedule = AramiscExamSchedule::where('exam_term_id', $exam_id)
    ...
    {
        return $this->hasMany('App\AramiscStudentPromotion', 'student_id', 'id');
    }
    ...
    public function getSessionsAttribute()
    {
        ...
        if (count($this->promotion) > 0) {
            ...
            $maxYear = AramiscAcademicYear::find($maxSession)->year ?? "";
            $minYear = AramiscAcademicYear::find($minSession)->year ?? "";
    ...
    {
        try {
            $session = AramiscSession::where('id', $session)->first();
    ...
    {
        $AramiscExamSetup = AramiscExamSetup::query();
        $eligible_subjects = universityFilter($AramiscExamSetup, $request)
                            ->where('exam_term_id', $exam_id)
    ...
    {
        return $this->hasMany(AramiscExamAttendanceChild::class, 'student_id');
    }
    ...
    public function DateWiseAttendances()
    {
        if (moduleStatusCheck('Univeristy')) {
            $request = request();
            return $this->hasOne(AramiscStudentAttendance::class, 'student_id')
