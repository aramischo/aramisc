...
@section('title')
@lang('student.student_aramiscAttendance')
@endsection
...
{{-- @php
$breadCrumbs = [
'h1'=>__('student.student_aramiscAttendance'),
'bgPages'=>[
                ...
                <div class="white-box">
                    <div class="row">
                        <div class="col-lg-12 d-flex align-items-center justify-content-between flex-wrap">
                            ...
                            <a href="{{route('student-aramiscAttendance-import')}}"
                                class="primary-btn small fix-gr-bg pull-right mb-15"><span
                                    class="ti-plus pr-2"></span>@lang('student.import_aramiscAttendance')</a>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-md-3 mt-30">
                            <div class="primary_input">
                                <label for="startDate">@lang('hr.aramiscAttendance_date')<span class="text-danger">
                                        *</span></label>
                                        ...
                                        <div class="col">
                                            <div class="">
                                                <input
                                                    ...
                                                    id="startDate" type="text" name="aramiscAttendance_date" autocomplete="off"
                                                    value="{{isset($date)? $date: date('m/d/Y')}}">
                                            </div>
                                        </div>
                                        <button class="btn-date" data-id="#aramiscAttendance_date" type="button">
                                            <label class="m-0 p-0" for="startDate">
                                                <i class="ti-calendar" id="aramiscAttendance_date"></i>
                                            </label>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="primary_datepicker_input">
                                    <div class="no-gutters input-right-icon">
                                        <div class="col">
                                            <div class="">
                                                <input
                                                    class="primary_input_field  primary_input_field date form-control{{ $errors->has('aramiscAttendance_date') ? ' is-invalid' : '' }}"
                                                    id="aramiscAttendance_date" type="text" name="aramiscAttendance_date"
                ...
                <div class="white-box">
                    <div class="row">
                        <div class="col-lg-12 no-gutters">
                            <div class="main-title">
                                @isset($search_info)
                                <h3 class="mb-15">@lang('student.student_aramiscAttendance') | <small>
                                        @if(moduleStatusCheck('University'))
                    ...
                    <div class="row mb-20">
                        <div class="col-lg-6  col-md-6 no-gutters text-md-left mark-holiday ">
                            ...
                            <form action="{{route('student-aramiscAttendance-holiday')}}" method="POST">
                                @csrf
                                <input type="hidden" name="purpose" value="mark">
                                <input type="hidden" name="class_id" value="{{$class_id}}">
                                <input type="hidden" name="section_id" value="{{$section_id}}">
    
                                <input type="hidden" name="aramiscAttendance_date" value="{{$date}}">
                                @if(moduleStatusCheck('University'))
                    ...
                    {{ Form::open(['class' => 'form-horizontal', 'route'=>'student-aramiscAttendance-store', 'method' => 'POST'])}}
                    <input type="hidden" name="date" class="aramiscAttendance_date" value="{{isset($date)? $date: ''}}">
                    <div class="row">
                        <div class="col-lg-12">
                            <x-table>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$student->studentDetail->admission_no}}
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}]"
                                                        value="{{$student->id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][student]"
                                                        value="{{$student->student_id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][class]"
                                                        value="{{$student->class_id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][section]"
                                                        value="{{$student->section_id}}">
                                                    @if(moduleStatusCheck('University'))
    
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][un_session_id]"
                                                        value="{{$student->un_session_id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][un_faculty_id]"
                                                        value="{{$student->un_faculty_id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][un_department_id]"
                                                        value="{{$student->un_department_id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][un_academic_id]"
                                                        value="{{$student->un_academic_id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][un_semester_id]"
                                                        value="{{$student->un_semester_id}}">
                                                    <input type="hidden"
                                                        name="aramiscAttendance[{{$student->id}}][un_semester_label_id]"
                                                        value="{{$student->un_semester_label_id}}">
                                                    <input type="hidden" name="aramiscAttendance[{{$student->id}}][un_section_id]"
                                                ...
                                                <td>
                                                    <div class="d-flex radio-btn-flex">
                                                        <div class="mr-20">
                                                            <input type="radio"
                                                                name="aramiscAttendance[{{$student->id}}][aramiscAttendance_type]"
                                                                id="aramiscAttendanceP{{$student->id}}" value="P"
                                                                class="common-radio aramiscAttendanceP aramiscAttendance_type"
                                                                {{ $student->studentDetail->DateWiseAttendances !=null ? ($student->studentDetail->DateWiseAttendances->aramiscAttendance_type == "P" ? 'checked' :'') : ($aramiscAttendance_type != "" ? '' :'checked') }}>
                                                            <label
                                                                for="aramiscAttendanceP{{$student->id}}">@lang('student.present')</label>
                                                        </div>
                                                        <div class="mr-20">
                                                            <input type="radio"
                                                                name="aramiscAttendance[{{$student->id}}][aramiscAttendance_type]"
                                                                id="aramiscAttendanceL{{$student->id}}" value="L"
                                                                class="common-radio aramiscAttendance_type"
                                                                {{ $student->studentDetail->DateWiseAttendances !=null ? ($student->studentDetail->DateWiseAttendances->aramiscAttendance_type == "L" ? 'checked' :''):''}}>
                                                            <label
                                                                for="aramiscAttendanceL{{$student->id}}">@lang('student.late')</label>
                                                        </div>
                                                        <div class="mr-20">
                                                            <input type="radio"
                                                                name="aramiscAttendance[{{$student->id}}][aramiscAttendance_type]"
                                                                id="aramiscAttendanceA{{$student->id}}" value="A"
                                                                class="common-radio aramiscAttendance_type"
                                                                {{$student->studentDetail->DateWiseAttendances !=null ? ($student->studentDetail->DateWiseAttendances->aramiscAttendance_type == "A" ? 'checked' :''):''}}>
                                                            <label
                                                                for="aramiscAttendanceA{{$student->id}}">@lang('student.absent')</label>
                                                        </div>
                                                        <div>
                                                            <input type="radio"
                                                                name="aramiscAttendance[{{$student->id}}][aramiscAttendance_type]"
                                                                id="aramiscAttendanceH{{$student->id}}" value="F"
                                                                class="common-radio aramiscAttendance_type"
                                                                {{$student->studentDetail->DateWiseAttendances !=null ? ($student->studentDetail->DateWiseAttendances->aramiscAttendance_type == "F" ? 'checked' :'') : ''}}>
                                                            <label
                                                                for="aramiscAttendanceH{{$student->id}}">@lang('student.half_day')</label>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="primary_input">
                                                        <input class="primary_input_field form-control note_{{$student->id}}"
                                                            cols="0" rows="2" name="aramiscAttendance[{{$student->id}}][note]"
