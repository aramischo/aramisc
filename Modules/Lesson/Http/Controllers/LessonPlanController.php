...
class LessonPlanController extends Controller
{
    ...
    public function lessonPlannerLesson($day, $teacher_id, $routine_id, $lesson_date)
    {
        ...
        try {
            ...
            $lessons = AramiscLesson::when(moduleStatusCheck('University'), function ($query) use ($routine) {
    ...
    public function addNewLessonPlan(Request $request)
    {
        ...
        try {
            ...
            $lesson = AramiscLesson::find($request->lesson);
            $lesson_id = $lesson->id;
    ...
    public function ViewlessonPlannerLesson($lessonPlan_id)
    {
        try {
            ...
            $room_id = 401;
            $lessons = AramiscLesson::where('class_id', $class_id)
    ...
    public function EditlessonPlannerLesson($lessonPlan_id)
    {
        try {
            ...
            $topic = AramiscLessonTopicDetail::where('lesson_id', $lessonPlanDetail->lesson_detail_id)
                ->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->get();
            $lessons = AramiscLesson::where('class_id', $class_id)
    ...
    public function changeWeek(Request $request, $teacher_id, $next_date)
    {
        ...
        $aramisc_weekends = AramiscWeekend::orderBy('order', 'ASC')->get();
        $teachers = AramiscStaff::where('role_id', 4)->get();
        return view('lesson::lessonPlan.lesson_planner', compact('period', 'dates', 'week_number', 'this_week', 'class_times', 'teacher_id', 'aramisc_weekends', 'teachers'));
    }
    ...
    public function topicOverview()
    {
        try {
            ...
            $topics_detail = AramiscLessonTopicDetail::with('lesson_title', 'lessonPlan', 'lessonPlan.lessonDetail')->get();
    ...
    public function topicOverviewSearch(Request $request)
    {
        try {
            ...
            $lessons = AramiscLesson::distinct('lesson_title')->get();
            $classes = AramiscClass::get();
            $lesson_ids = AramiscLessonTopic::when($class_id, function ($q) use ($class_id) {
            ...
            $topics_detail = AramiscLessonTopicDetail::whereIn('lesson_id', $lesson_ids)->get();
            $selected = [];
    ...
    public function manageLessonPlanner()
    {
        ...
        try {
            ...
            $lessons = AramiscLesson::get();
            $topics = AramiscLessonTopic::get();
    ...
    public function completeStatus(Request $request)
    {
        ...
        try {
            ...
            $statusUpdate = AramiscLessonTopicDetail::where('id', $topic_id)->first();
    ...
    public function teacherLessonPlanOverview()
    {
        if (Auth::user()->role_id == 4) {
            try {
                ...
                return view('lesson::teacher.teacher_lesson_plan_overview', compact('dates', 'this_week', 'class_times', 'teacher_id', 'aramisc_weekends', 'teachers'));
            } catch (\Exception $e) {
    ...
    public function loadDefault()
    {
        ...
        $data['aramisc_weekends'] = AramiscWeekend::with('teacherClassRoutineAdmin')->where('school_id', Auth::user()->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
