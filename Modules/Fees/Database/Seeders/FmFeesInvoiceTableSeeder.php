...
class FmFeesInvoiceTableSeeder extends Seeder
{
    ...
    public function run($school_id, $academic_id, $count = 5)
    {
        ...
        $aramiscStudentIds = StudentRecord::select('id', 'class_id', 'student_id')->get();

        FmFeesGroup::factory()->times($count)->create($school_academic)->each(function ($feesGroup) use ($school_academic, $aramiscStudentIds) {
            FmFeesType::factory()->times(5)->create(array_merge([
                'fees_group_id' => $feesGroup->id,
            ], $school_academic))->each(function ($feesTypes) use ($school_academic, $aramiscStudentIds) {
                foreach ($aramiscStudentIds as $id) {
